...
status: Reconcile Directional-Contrast Cohort v1 -- Settlement Completion Pass WAIT-ONLY 2026-06-22 -- settlement gate not met. All 10 cohort games (200013-200022) StatsAPI Preview/Scheduled, 0 in progress; first pitch tonight 18:10-19:45Z, expect Final ~2026-06-23T04:00Z+. 0 reconciled, nothing written, totals unchanged 35/35. Cohort intact and reconcile-eligible once Final (identity-bearing mlb_statsapi+gamePk, active, completed, lean home 5/away 3/null 2). No correct/incorrect/inconclusive this pass. Home-vs-away lean result, null-lean handling: produced on the settlement-completion pass (grade on structured LeanSide; null leans 200016/200019 -> inconclusive). No code/model/generation/confidence/posture/lean/buyer/source-depth/reconciliation change. dai vault capture commit a8783b1 pushed before this pass; this wait-only doc committed (not pushed). Files: reconcile-directional-contrast-cohort-v1-settlement-pass.md. Next: rerun this slice (settlement-completion) after Final -> target 45/45, report home-lean vs away-lean accuracy, then Reconciled-Outcome Calibration Read v2.

---

status: Reconcile Directional-Contrast Cohort v1 -- Settlement Completion Pass STILL WAIT-ONLY 2026-06-22 (re-check 2) -- rerun re-queried StatsAPI at 11:59 EDT / 15:59Z; all 10 cohort games (200013-200022) still Preview/Scheduled, 0 in progress. First pitches are 18:10-19:45Z TODAY (~2-4h away); games not Final until ~2026-06-23T04:00Z+. The "should be settled" premise did not hold yet. 0 reconciled, nothing written, totals unchanged 35/35; pre-state re-verified (cohort 0 outcome/0 eval, identity intact). Pushed prior wait-only commit 21fc397 before re-check. No code/model/confidence/posture/lean/buyer/source-depth/reconciliation change. Doc updated with gate re-check log (committed, not pushed). Next: rerun this slice after ~2026-06-23T04:00Z when games are Final -> target 45/45, grade on structured LeanSide, report home-lean vs away-lean accuracy.

---

status: Market Odds Depth v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). Deepened the market signal from single-book current run line to multi-book moneyline+spread+total via the existing odds_api /odds endpoint. New pure module MarketDepth.cs (book count, consensus side, disagreement range, median implied prob, IsEnriched, AgreementWithLean) + 20 unit tests; BaseballMarketContext gained nullable depth fields; SourceDepthEvaluator now classifies market_odds enriched (>=2 books w/ moneyline consensus) vs shallow, REUSING existing vocabulary (no new tier); Python analyzer injects a market-depth block (data only, no prompt-doctrine change). Fail-soft: single-book/no-h2h stays shallow, never fails generation. Full suite 773/0 green; py_compile clean. LIVE proof (run d2b5433e, real MLB): market_odds=enriched "9 books, consensus away, median home win prob 48%, disagreement 2%"; lean away @ 0.8 (richer input moved model off flat 0.75 -- source variance, NOT a confidence claim). DEFERRED: lean-vs-market agreement as a surfaced field (pure helper done+tested; needs read-time projection) -> Market Agreement Projection v1; historical line movement (paid endpoint); NBA/NFL multi-book extension. No deterministic lean/confidence/posture-logic, prompt-doctrine, reconciliation-logic, buyer-UI-template, or migration change. Doc: 04 Products/sports-v1/market-odds-depth-v1.md. dai + dai-vault committed separately, not pushed. Note: live verification created MLB run ~200023 (d2b5433e) + a harness report in the vault. Next: Market Agreement Projection v1, or fold depth into Reconciled-Outcome Calibration Read v2 after the 200013-200022 cohort settles.

---

status: gRPC Inference Orchestration Boundary v1 complete 2026-06-22 -- directional architecture plan, vault-only, no code/proto/runtime change. Key decision: gRPC stays UNBUILT; the first (future) gRPC candidate is the .NET DevCore.Api orchestrator <-> inference worker boundary (primary analysis / verifier findings / synthesis), justified only once inference becomes multi-stage across processes (extends next-platform-architecture-plan.md section 5; reconciles with the existing unused FastAPI 50051 / assist.proto scaffold). Source ingestion stays REST/JSON provider adapters; Angular stays HTTP; reconciliation stays HTTP; dormant protocol nodes stay in-process. Conceptual contracts defined (DecisionAnalysis/ArtifactVerification/Synthesis req+resp, SourceSignalEnvelope, VerificationFinding, SynthesisTrace) as DTOs-now/protobuf-later; verifier emits findings only and never flips structured LeanSide; synthesis never fabricates missing evidence. Non-goals: implement gRPC, migrate FastAPI boundary, gRPC-Web, streaming, agent-worker runtime, Protocol Node Runner / Probe Refresh activation, exposing 50051. Doc: 02 Platform/architecture/inference/grpc-inference-orchestration-boundary-v1.md (new inference/ folder). dai untouched. Committed dai-vault, not pushed. Next: keep building MLB source-depth now; later Inference Worker Contract v1 (C# record contracts) once source envelopes + verifier needs stabilize.

---

status: Cross-Sport Source Component Architecture v1 complete 2026-06-22 -- directional architecture plan, vault-only, no code/runtime change. Key decision: "reusable signal contract, sport-specific adapter" -- the reusable categories already exist latently as SourceSignalTaxonomy.SourceGroups (market_odds, lineup_injury, rest_travel, bullpen_availability, weather_park, team_form, starting_pitching, market_movement); this plan promotes that taxonomy into an explicit source-component model (SourceComponent/SourceAdapter/SourceSignalEnvelope + status/freshness/provenance), reusing the SourceSignalEnvelope shape from the gRPC inference doc. Cross-sport categories defined (MarketContext, AvailabilityContext, ScheduleRestContext, FatigueContext, VenueEnvironmentContext, ParticipantFormContext, MatchupContext, SourceFreshnessContext) each mapped to a canonical group; new "stale" status proposed (freshness-derived; not in code yet). Ranked source roadmap; biggest payoff = AvailabilityContext grounds the standing NamedRisk-Ungrounded weakness. Non-goals: build framework now, gRPC, confidence tuning, prompt/buyer/reconciliation/migration changes, provider marketplace. Doc: 02 Platform/architecture/sources/cross-sport-source-component-architecture-v1.md (new sources/ folder). dai untouched. Committed dai-vault, not pushed. Recommended next: Cross-Sport Source Envelope v1 (introduce the envelope, retrofit existing market + starter signals onto it before adding new providers), then Availability Context v1.

---

status: Cross-Sport Source Envelope v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). Added reusable SourceSignalEnvelope (SignalKey/SourceGroup/Sport/Provider/Status/Depth/Observed/UpdatedAtUtc/SourceRef/summaries) + pure SourceEnvelopeBuilder that wraps the grounded MLB market + starter signals, using the authoritative SourceDepthRecord[] for depth so envelopes and SourceDepth cannot diverge. Statuses observed/proxy/missing/unavailable/not_attempted/not_applicable; "stale" intentionally deferred (freshness modeled, no threshold yet). Wired additively (mirrors SourceDepth): SportsRetriever builds -> SportsRetrievalOutput.SourceEnvelopes -> SportsComposer persists (success+failed) -> AgentRunExecutionResult + artifact DTO + GET /artifact projection. SourceDepth behavior byte-for-byte unchanged; no confidence/posture/lean/prompt/buyer/reconciliation/migration change; no new providers; analyzer prompt-injection NOT changed (deferred). Tests: new SourceEnvelopeBuilderTests 8/8; full DevCore.Api.Tests 781/0 (was 773, +8, no regressions). LIVE run dab5433e: envelopes present for starting_pitching (observed/enriched/mlb_statsapi) + market_odds (observed/enriched/odds_api, updatedAt set); sourceDepth unchanged (both enriched); lean away @0.8. Docs: 02 Platform/architecture/sources/cross-sport-source-envelope-v1.md. dai + dai-vault committed separately, not pushed. Note: live verification created MLB run dab5433e. Next: Availability Context v1 (MLB lineup/injury adapter -> lineup_injury envelope; biggest named-risk-grounding win), or Market Agreement Projection v1, or reconcile 200013-200022 once Final.

---

status: Availability Context v1 -- MLB First Adapter complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). First new source lane on the SourceSignalEnvelope contract: a lineup_injury envelope via a new gateway tool availability.mlb.lineup (MlbAvailabilityClient -> statsapi boxscore battingOrder presence, keyed by the gamePk from starter retrieval). Statuses observed (both lineups posted) / proxy (one) / missing (none posted) / unavailable (provider failure). SourceDepth lineup_injury=shallow when confirmed, never enriched; no record when missing so breadth/sufficiency/confidence untouched (availability not added to GroundedSignals -- observed-only). Lineup confirmation only; player injury inference intentionally excluded (no reliable free source). Fail-soft: missing/partial lineup never fails the run. Tests: +4 envelope, +3 SourceDepth; fixed 6 SportsRetrieverTests by registering the new tool in the test gateway (root cause: unregistered tool, not architecture). Full DevCore.Api.Tests 788/0 (was 781, +7). LIVE run dbb5433e: envelopes = starting_pitching observed/enriched + market_odds observed/enriched + lineup_injury proxy/shallow ("one starting lineup confirmed", fresh); sourceDepth lineup_injury=shallow; lean away @0.8 unchanged. No confidence/posture/lean/prompt/buyer/reconciliation/migration change; no new paid providers. Deferred: player injuries, analyzer prompt-injection, stale status, NBA/NFL adapters, named-risk-grounding integration. Docs: 04 Products/sports-v1/availability-context-v1.md. dai + dai-vault committed separately, not pushed. Note: live verification created MLB run dbb5433e. Next: Schedule/Fatigue Context v1, or Market Agreement Projection v1, or reconcile 200013-200022 once Final.

---

status: Schedule/Fatigue Context v1 -- MLB Bullpen Adapter complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). Second new source lane on the SourceSignalEnvelope contract: a bullpen_availability envelope via a new gateway tool fatigue.mlb.bullpen (MlbBullpenFatigueClient -> statsapi teams + recent schedule + boxscore; relievers used last game = pitchers minus starter, per team; names heavier-workload side). Status proxy when resolved (derived, never observed), unavailable otherwise. SourceDepth bullpen_availability=shallow when resolved, never enriched; no record when unavailable so breadth/sufficiency/confidence untouched (observed-only, not added to GroundedSignals). Team-level only; no player reliever projection, no quality claims. Fail-soft: unresolved -> unavailable, run never fails. Basketball rest->rest_travel envelope mapping DEFERRED (documented). Tests: +3 envelope, +2 SourceDepth, SportsRetrieverTests gateway updated (registered the tool, learned from last slice -> no breakage). Full DevCore.Api.Tests 793/0 (was 788, +5). LIVE run deb5433e: envelopes = starting_pitching observed/enriched + market_odds observed/enriched + lineup_injury proxy/shallow + bullpen_availability proxy/shallow ("away bullpen more taxed recently"); sourceDepth bullpen_availability=shallow; lean away @0.8 unchanged. No confidence/posture/lean/prompt/buyer/reconciliation/migration change; no new paid providers. Docs: 04 Products/sports-v1/schedule-fatigue-context-v1.md. dai + dai-vault committed separately, not pushed. Note: live verification created MLB run deb5433e. Next: Named-Risk Grounding Integration v1 (map availability/fatigue named-risks onto the now-groundable lanes), or VenueEnvironment Context v1, or reconcile 200013-200022 once Final.

---

status: Market Memory Layer v1 -- Architecture and Requirements complete 2026-06-22 -- feature architecture + requirements plan, vault-only, no code/schema/runtime change. Defines DAI's persisted odds market-history layer: own normalized snapshots in-house (provider API stays raw source), observe/persist/query first, derive movement/close/beat-close read-only, expose to synthesis + calibration only after evidence. Domain model (MarketEventIdentity, MarketSnapshotBatch, MarketBookLine, MarketConsensusSnapshot, MarketMovementSummary, ArtifactMarketSnapshotLink, ClosingMarketSnapshot), minimum persisted ledger, derived metrics, capture strategy (run-triggered v1 only, scheduled/near-close later, no minute polling), open/current/latest/close semantics, identity joins, dedupe, retention, observability, risks. KEY DECISION/finding: the code currently DISCARDS the odds_api event id for MLB (keeps only statsapi gamePk) -- market memory must additionally persist the odds event id to re-join future odds fetches; the ledger carries BOTH (gamePk joins AgentRun+reconciliation, odds event id joins successive odds fetches). 7-slice roadmap; immediate next = Market Snapshot Ledger v1 (write run-linked normalized snapshots at generation, near-zero provider cost, fail-soft, additive migration). Non-goals: implementation, migrations, polling, dashboards, buyer CLV claims, confidence tuning, decision changes, paid historical provider, gRPC, full warehouse. Doc: 02 Platform/architecture/sources/market-memory-layer-v1.md. dai untouched. Committed dai-vault, not pushed. Repo hygiene: untracked harness verification artifacts from this session's live runs (3 calibration reports + 3 artifact JSONs, dab5433e/dbb5433e/deb5433e) left uncommitted -- not part of this slice. Next: Market Snapshot Ledger v1.

---

status: Market Snapshot Ledger v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). First persisted market-memory layer: writes normalized odds snapshots during artifact generation, reusing the multi-book readings odds_api already returns (near-zero extra cost). New entities MarketSnapshotBatch + MarketBookLine (DevCore.Domain) + additive migration 20260622205819_AddMarketSnapshotLedger (2 tables, indexes, unique DedupeKey). Pure MarketSnapshotNormalizer (readings -> h2h/spreads/totals lines + deterministic dedupe key excluding fetchedAt) + IMarketSnapshotStore (dedupe by key). Capture path: OddsMarketClient surfaces per-book readings + providerUpdatedAt -> SportsRetriever builds MarketSnapshotDraft (captures the odds_api event id, previously DISCARDED on the MLB path) -> AgentRunService -> controller persists after run save, linked by AgentRunId, fail-soft (try/catch, never fails the artifact). KEY: batch carries BOTH ProviderEventId (odds event id, rejoins future odds fetches) and ExternalGameId (gamePk, joins AgentRun+reconciliation); gamePk stays the reconciliation key. Dedupe: identical state -> no dup; price/point/providerUpdatedAt change -> new batch. No movement/close/polling/synthesis/confidence/lean/posture/buyer/reconciliation change. Tests: +7 (normalizer + store in-memory); full DevCore.Api.Tests 800/0 (was 793). Migration applied to dev DB. LIVE run e2b5433e: batch persisted -- Provider odds_api, ProviderEventId 7eeb2a9f..., ExternalGameId 824261 (gamePk), LinkedAgentRunId set, BookCount 9, consensus away, markets h2h,spreads,totals; book lines h2h 18 / spreads 18 / totals 9; market_odds envelope still observed; lean away @0.8 unchanged. Docs: 04 Products/sports-v1/market-snapshot-ledger-v1.md. dai + dai-vault committed separately, not pushed. Note: live run created run e2b5433e + a snapshot batch. Untracked harness byproducts left outside this commit (referenced by prior docs; user's disposition). Next: Market Snapshot Query v1.

---

status: Market Snapshot Query v1 complete 2026-06-22 -- IMPLEMENTED + verified (TDD). Minimal internal read-only surface over the market snapshot ledger. New MarketSnapshotQuery (composable filters: LinkedAgentRunId, ExternalGameId, ProviderEventId, Provider, MarketType, FetchedFrom/To) + read DTOs (MarketSnapshotBatchView/MarketBookLineView) + IMarketSnapshotQueryService over AppDbContext (AsNoTracking, no provider calls). MarketType filter restricts lines + excludes batches lacking that market. Deterministic ordering: batches by FetchedAtUtc then key; lines by MarketType,Side,Bookmaker,Point,Price,ProviderUpdatedAt. Empty result = success/empty list. No HTTP endpoint (internal/dev). NO movement/close/first-latest, no provider/polling, no write-path/artifact/prompt/buyer/lean/confidence/posture/reconciliation change. Tests: +7 (in-memory) covering by-run/by-gameid/by-eventid/marketType/time-range/ordering/empty; existing ledger tests preserved; full DevCore.Api.Tests 807/0 (was 800). Verified live: batch from run e2b5433e reachable by LinkedAgentRunId + ExternalGameId 824261 + ProviderEventId 7eeb2a9f..., h2h lines in deterministic order. Docs: 04 Products/sports-v1/market-snapshot-query-v1.md. dai + dai-vault committed separately, not pushed. Next: Market Movement Derivation v1.

---

status: Market Movement Derivation v1 complete 2026-06-22 -- IMPLEMENTED + verified (TDD). Read-only derivation over persisted snapshot batches. Pure MarketMovementCalculator (first/latest observed by FetchedAtUtc; consensus first/latest + flip; per-book line movement for bookmaker+marketType+side pairs in BOTH first and latest -- price delta for h2h when prices exist, point delta for spreads/totals; unpaired omitted, no fake deltas) + explicit DTOs (MarketMovement/MarketLineMovement) + MarketMovementDerivationService reading through IMarketSnapshotQueryService. closeObserved = true only with a near_close capture reason (constant reserved; no writer emits it) -> FALSE today; latest is never treated as close. Boring insufficient data: 0 batches -> null; 1 batch -> firstObserved only, no delta. No provider calls, no close capture, no polling, no write-path/artifact/prompt/buyer/lean/confidence/posture/reconciliation change, no endpoint. Tests: +13 (10 pure calculator + 3 in-memory service); existing ledger + query tests preserved; full DevCore.Api.Tests 820/0 (was 807). Verified live: ExternalGameId 824261 (run e2b5433e) has 1 batch + 0 near_close -> service reports BatchCount 1, no delta, closeObserved false. Docs: 04 Products/sports-v1/market-movement-derivation-v1.md. dai + dai-vault committed separately, not pushed. Next: MarketMovementContext Envelope v1.

---

status: MarketMovementContext Envelope v1 complete 2026-06-22 -- IMPLEMENTED + verified (TDD). Observed-only market_movement source envelope built from the read-only movement derivation. New MarketMovementEnvelope typed DTO (batch count, first/latest observed, closeObserved, consensus first/latest+flip, line movement count, capped deterministic representative line movements, provider/eventId) + lane-scoped MarketMovementStatuses (observed/insufficient_history/unavailable, does NOT extend shared SourceSignalStatuses) + pure MarketMovementEnvelopeBuilder + thin MarketMovementEnvelopeService over IMarketMovementDerivationService. Statuses: 0 batches->unavailable; 1 batch->insufficient_history (no movement fabricated); >=2->observed. closeObserved carried from derivation -- true only with near_close capture (none today); latest never treated as close; language "observed movement", never closing-line value. MarketMovement gained additive Provider+ProviderEventId (latest batch) -- no delta-semantics change. Internal service, NOT wired to artifact/buyer (consistent with query/derivation slices; surfacing deferred). No provider calls, no close capture, no write-path/movement-semantics/artifact/prompt/buyer/lean/confidence/posture/reconciliation change, no endpoint. Tests: +10 (7 builder + 3 in-memory service); existing ledger/query/calculator/derivation tests preserved; full DevCore.Api.Tests 830/0 (was 820). Verified live: ExternalGameId 824261 (run e2b5433e) has 1 batch + 0 near_close -> envelope = insufficient_history, BatchCount 1, no movement, closeObserved false. Docs: 04 Products/sports-v1/market-movement-context-envelope-v1.md. dai + dai-vault committed separately, not pushed. Next: Closing Snapshot Capture v1.

---

status: Closing Snapshot Capture v1 complete 2026-06-22 -- IMPLEMENTED + verified (TDD). Internal near-close capture path: ClosingSnapshotCaptureService fetches current odds via OddsMarketClient, gates pregame, and writes a near_close snapshot batch through the existing ledger store (reusing per-book h2h/spreads/totals normalization). Results: written / deduped / skipped_started / skipped_no_market. Pregame-only: now<commence -> write; now>=commence -> skipped_started (never in-play); no odds event -> skipped_no_market (no stale close; postponed/off-board). Preserves odds_api event id + gamePk (when provided); no AgentRun required (optional link). Dedupe key now includes captureReason so near_close never collides with artifact_generation identical lines. MarketCaptureReasons.NearClose now actually written (was reserved). closeObserved: false without near_close, true with -- derivation + envelope reflect it; latest never treated as close; no CLV. No endpoint, no scheduler, no artifact/buyer/prompt/confidence/posture/reconciliation change. Tests: +6 (full odds-parse path + in-memory): pregame writes near_close w/ lines + identities + no-run; started->skipped; no-market->skipped; duplicate->deduped; near_close vs identical artifact_generation both persist; end-to-end closeObserved false->true after capture (derivation + envelope). Existing ledger/query/derivation/envelope tests preserved; full DevCore.Api.Tests 836/0 (was 830). Live: ledger has 0 near_close today -> closeObserved correctly false; 824261 past commence -> would skip_started; live capture trigger skipped (no endpoint + game started), behavior proven by deterministic tests. Docs: 04 Products/sports-v1/closing-snapshot-capture-v1.md. dai + dai-vault committed separately, not pushed. Next: Market Snapshot Scheduler v1 (or Artifact-vs-Close Calibration v1).

---

status: Near-Close Capture Trigger v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). Makes near-close capture operational. NearCloseCaptureTrigger service iterates explicit pregame targets, invokes IClosingSnapshotCaptureService per target, applies an exists pre-flight skip, aggregates counts. Dev-gated endpoint POST /api/agent-runs/near-close-capture (tenant via identity.ResolveAsync like reconcile); request {games:[{competition,homeTeam,awayTeam,gameDate,externalGameId?,agentRunId?}],force}. Explicit-input selection (ledger team-ref slugs can't drive the provider team-name odds match). Close semantics delegated to capture service (pregame writes, started/no-market skip; latest never close; closeObserved only near_close). Idempotency: trigger exists-skip (when externalGameId+!force) + capture-service line dedupe. Output: attempted/written/deduped/skippedStarted/skippedNoMarket/skippedExists + per-game results. NOT a scheduler/polling/background job; no artifact/buyer/prompt/confidence/posture/reconciliation change; no CLV. Tests +5 (in-memory + fake odds): eligible->written, started->skipped, no-market->skipped, existing+!force->skipped_exists, summary aggregate; full DevCore.Api.Tests 841/0 (was 836). LIVE: POST for 3 upcoming MLB games -> written=3, ledger gained 3 near_close batches (odds_api, distinct event ids, per-book h2h/spreads/totals lines 45/10/26); re-run -> written=0 deduped=3 (idempotent). closeObserved now real; gamePk-keyed flip covered by tests (only gamePk-tagged game 824261 is past commence). Live demo targets used team/date (no gamePk) -> near_close carries odds event id, null gamePk; pass externalGameId to tie to a game's movement. Docs: 04 Products/sports-v1/near-close-capture-trigger-v1.md. dai + dai-vault committed separately, not pushed. Note: live trigger wrote 3 near_close batches to dev DB. Vault hygiene: 8 untracked harness byproducts referenced by prior docs -- recommend a SEPARATE hygiene commit; kept out of this slice. Next: Market Snapshot Scheduler v1 (or Artifact-vs-Close Calibration v1).

---

status: Tracked Near-Close Target Resolver v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). Fixes prior slice's null-gamePk gap: near_close now joins artifact_generation by full identity. New TrackedNearCloseTargetResolver finds this-tenant artifact_generation odds_api batches with a ProviderEventId + future commence (one per event id, latest), carrying gamePk/AgentRun/commence/eastern gameDate/AlreadyHasNearClose; batches without provider event id are not eligible. OddsMarketClient.GetBaseballSpreadByEventIdAsync matches the odds event by id (no team-name matching); ClosingSnapshotCaptureService.CaptureNearCloseByEventIdAsync shares the gate+persist core. New dev-gated endpoint POST /api/agent-runs/near-close-capture/tracked {force}; NearCloseCaptureTrigger.CaptureTrackedAsync resolves + captures by event id; result adds eligible + skippedMissingProviderEventId. Idempotency: resolver flags AlreadyHasNearClose -> skipped_exists unless force; capture dedupe otherwise. No team-name matching, no scheduler, no artifact/buyer/prompt/confidence/posture/reconciliation change, no CLV. Tests +7 (5 resolver + 2 tracked trigger); existing trigger tests updated for new ctor; full DevCore.Api.Tests 848/0 (was 841). LIVE: generated artifact (run e5b5433e, gamePk 824585, event id 22bf5f58...) -> tracked trigger eligible=1 written=1 -> near_close batch shares SAME ProviderEventId + ExternalGameId 824585 + LinkedAgentRunId + 45 lines; closeObserved=TRUE for 824585. Docs: 04 Products/sports-v1/tracked-near-close-target-resolver-v1.md. dai + dai-vault committed separately, not pushed. Note: live verification created run e5b5433e + artifact_generation + near_close batches for 824585. Vault hygiene: 8 untracked byproducts -- recommend separate hygiene commit; kept out. Next: Artifact-vs-Close Calibration v1 (now unblocked) or Market Snapshot Scheduler v1.

---

status: Close-Market Confirmation Read v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD); + Decision Freshness Protocol planning seed. Read-only comparison of artifact_generation vs near_close market state relative to the artifact lean. Pure CloseMarketConfirmationCalculator (lean-side implied prob movement >= +/-1pp -> moved_toward/against_lean; fallback consensus flip; else neutral/not_comparable) + CloseMarketConfirmationService (joins run lean + artifact_generation batch + near_close by ProviderEventId+gamePk; optional read-only ArtifactEvalStatus) + dev-gated GET /api/agent-runs/{id}/close-market-confirmation. Status model: observed/no_generation_snapshot/no_near_close_snapshot/no_artifact_lean/not_comparable/unavailable. NOT full CLV: TrueClvEvaluable=false + ClvBoundaryNote (artifact has directional lean, not formal market selection type/side/line/price). Buyer-safe phrases ("Market moved toward/against this read", "stayed mostly neutral", "No close-market comparison available yet") -- NOT surfaced to buyer UI. Optional outcome join is read-only (eval row count unchanged; no reconciliation mutation). No prompt/confidence/lean/posture/reconciliation/buyer/artifact-JSON/scheduler change. Tests +13 (7 calculator + 6 service); full DevCore.Api.Tests 861/0 (was 848). LIVE: GET for run e5b5433e (gamePk 824585) -> status=observed, lean=away, direction=neutral (lean prob 0.524 at gen and close -> no movement), joined by same ProviderEventId 22bf5f58... + gamePk + both batch ids, trueClvEvaluable=false, summary "Market stayed mostly neutral." Decision Freshness Protocol seed added (02 Platform/architecture/sources/decision-freshness-protocol-seed-v1.md): baseline artifact -> refreshed sources -> source deltas -> market support -> line quality -> freshness summary; all runtime behavior deferred. Docs: 04 Products/sports-v1/close-market-confirmation-read-v1.md. dai + dai-vault committed separately, not pushed. Vault hygiene: 10 untracked byproducts -- recommend separate hygiene commit; kept out. Next: Decision Freshness Architecture v1 or Outcome-Calibrated Market Confirmation v1.

---

status: Decision Freshness Architecture v1 complete 2026-06-22 -- architecture + requirements plan, vault-only, no code/runtime change. Designs a read-only freshness layer: baseline artifact + refreshed source state + source deltas + buyer-safe summary. Feature boundary: freshness is NOT confidence, NOT reconciliation, NOT full CLV, NOT auto-rerun, NOT buyer UI yet. Builds on existing infra: persisted SourceEnvelopes (market_odds, market_movement, starting_pitching, lineup_injury, bullpen_availability) + market-memory chain + Close-Market Confirmation Read v1 (the first market-support leg). Domain model (DecisionFreshnessRequest/Baseline/Refresh, SourceDelta + Market/Line/Availability/Fatigue/Starter/Venue deltas, DecisionFreshnessResult, BuyerSafeFreshnessSummary). Baseline = immutable persisted envelopes + artifact_generation batch + LeanSide + generatedAt + run; refresh compares against baseline, never mutates. Source delta classes: unchanged/strengthened/weakened/newly_available/no_longer_available/stale/unavailable/not_applicable/not_comparable. Status model: fresh_supported/fresh_neutral/weakened/stale/unavailable/not_comparable/insufficient_history (diagnostic, no decision mutation). Buyer-safe vocabulary defined (market support, line quality, source updates; no CLV/sharp/lock/basis-points/event-ids). No confidence/lean change until outcome-calibrated. 7-slice roadmap (Read Model -> Source Delta Calculator -> Refresh Trigger -> Artifact Projection -> Buyer-Safe Copy -> Refresh/Rerun UX -> Freshness Calibration). Recommended next = Decision Freshness Read Model v1 (uses persisted data + close-market confirmation, no provider calls, no buyer UI). Doc: 02 Platform/architecture/sources/decision-freshness-architecture-v1.md. dai untouched. Committed dai-vault, not pushed. Next: Decision Freshness Read Model v1.

---

status: Decision Freshness Read Model v1 complete 2026-06-22 -- IMPLEMENTED + live-verified (TDD). First Decision Freshness read: read-only DecisionFreshnessResult combining a run's persisted baseline (LeanSide, StartedUtc, SourceEnvelopes from OutputJson) with Close-Market Confirmation Read v1. Pure DecisionFreshnessCalculator maps close-market -> overall status (observed+toward->fresh_supported; against->weakened; neutral->fresh_neutral; no_near_close->insufficient_history; no_artifact_lean/not_comparable->not_comparable; no_generation/unavailable->unavailable) and summarizes baseline source-envelope lanes as DecisionFreshnessSourceDelta (one per persisted envelope + a derived market_movement delta from close state). DecisionFreshnessService loads run + fail-soft deserializes envelopes + calls ICloseMarketConfirmationService; read-only, no writes. Dev-gated GET /api/agent-runs/{id}/decision-freshness (tenant-scoped). Baseline-only: NO provider fetch, NO refresh, NO regeneration; v1 never classifies a lane weakened and never emits stale (reserved for refresh slice). TrueClvEvaluable stays false (carried from close-market; artifact has a directional lean, not a formal market selection). Buyer-safe language only; tests assert forbidden terms absent (CLV, sharp, lock, basis point, implied probability, guaranteed, event id). No prompt/buyer-UI/artifact-JSON/confidence/lean/posture/reconciliation/scheduler/gRPC change. Tests +10 (8 pure calculator + 2 in-memory service); full DevCore.Api.Tests 871/0 (was 861). LIVE: GET for run e5b5433e (gamePk 824585) -> overallStatus=fresh_neutral, lean away, trueClvEvaluable=false, close-market confirmation embedded (observed/neutral), five source deltas (market_odds observed/enriched, starting_pitching observed/enriched, lineup_injury observed/shallow, bullpen_availability proxy/shallow, market_movement observed) all unchanged, sourceUpdate "Source context is available for: bullpen, lineup, market, starting pitching." Docs: 04 Products/sports-v1/decision-freshness-read-model-v1.md. dai + dai-vault committed separately, not pushed. Next: Source Delta Calculator v1 (baseline-vs-refreshed deltas) or Refresh Trigger v1.

---

status: Graphify Development Navigation Doctrine v1 complete 2026-06-23 -- docs/vault only, no code/runtime change. Documents how DAI uses Graphify as a LOCAL development navigation layer (AST-only, no LLM, no API cost, no code upload) and ties it to the factory model: platform=factory, agents=workers, graphify=factory-floor map. New vault doctrine doc `02 Platform/architecture/devtooling/graphify-development-navigation-doctrine-v1.md` covers purpose; positioning (navigation layer; not source of truth, not runtime dependency, not buyer-facing, not a permission expansion); strategic fit; approved uses (orientation before unfamiliar slices, blast-radius before DTO/artifact/handler/read-model/contract changes, cross-stack contract discovery, hotspot location, comparing findings vs source/tests/vault); disallowed uses (treating graph as authoritative, behavior claims from graph alone, replacing tests/source/contracts, auto-generated unverified changes, semantic/cloud extraction without approval, running on dai-vault, committing graph artifacts); slice workflow; truth hierarchy (runtime+tests > source > contracts/vault > handoffs > graphify as navigation evidence only); current constraints (unnamed communities, name-based cross-language false positives, test-inflated god nodes); deferred decisions (LLM extraction, vault graphing, combined workspace graph, git hooks, CI generation, sharing graph with other agents); and 3 initial graph-assisted investigations (cross-stack sports contract coupling; AppDbContext blast radius; high-degree python helpers `_parse_response`/`_make_raw`), each with why/commands/verification/expected outcome. `README.md` links it under a new "Development tooling" section. What did NOT change: no application source, no runtime behavior, no semantic/cloud/LLM extraction, no graphify run on `<DAI_VAULT_ROOT>`, no `graphify-out/` generated or committed, `.claude/settings.json` untouched. Repo states: `<DAI_REPO_ROOT>` main HEAD 0908cf8 (unchanged this slice, clean); `<DAI_VAULT_ROOT>` main, clean before -> this slice adds the graphify doctrine doc + README reference + this handoff entry. Committed `<DAI_VAULT_ROOT>` only (docs(devtools): document graphify navigation doctrine), not pushed; `<DAI_REPO_ROOT>` untouched. Next: Graph-Assisted Contract Coupling Review v1 -- use Graphify to investigate the C#/.NET <-> Python FastAPI <-> Angular sports contract coupling surfaced by the graph, then verify against actual source and tests.

---

status: DAI Documentation Skill and Topic Slice System v1 complete 2026-06-23 -- skills + docs only, no code/runtime change. Created the `dai-docs-architect` skill (`<DAI_REPO_ROOT>/.claude/skills/dai-docs-architect/SKILL.md`): turns DAI concepts into durable vault doctrine -- enforces one-doctrine-per-doc decomposition, a standard topic template (purpose, problem, strategic fit, mental model, what it is / is not, approved + disallowed uses, workflow impact, truth hierarchy, source refs to verify, example, agent prompt guidance, acceptance criteria, risks/failure modes, deferred decisions, related, next slice), a 9-step documentation-slice workflow (read existing -> verify source only as needed -> bound the topic -> write -> link from an index -> update handoff -> verify diff -> commit docs-only -> handoff), and a verify-before-assert / doctrine-vs-deferred quality bar. Inspected existing skills first (rule: no duplication): `dai-write-skill` authors skills, `dai-grill-with-vault` interrogates plans, `dai-agent-handoff` writes handoffs -- none cover writing durable topic doctrine, so created new rather than duplicate. Registered in `dai-skill-router` (added a routing rule + pair-with). Updated the vault skills inventory (skill count 10 -> 11) and added `06 Execution/backlog/documentation-doctrine-backlog-v1.md` ranking 10 topics by state/priority; P1 next docs = Agent Slice Workflow Doctrine, Source Depth & Evidence Sufficiency Doctrine, Tool Gateway & Agent Permissions Doctrine. What did NOT change: no application source, no runtime behavior, no semantic/cloud/LLM extraction, no graphify run, no `graphify-out/`, `.claude/settings.json` untouched. Repo states: `<DAI_REPO_ROOT>` main 0908cf8 before -> commit adds the `dai-docs-architect` SKILL.md + `dai-skill-router` edit; `<DAI_VAULT_ROOT>` main 9ab04aa before -> commit adds the backlog doc + inventory update + this handoff entry. Committed both repos separately (docs(skills): add dai documentation architect workflow), not pushed. Next: Agent Slice Workflow Doctrine v1 -- write the first P1 backlog doc using `dai-docs-architect`.

---

status: DAI Code Review Skill v1 complete 2026-06-23 -- skills + docs only, no code/runtime change. Created `dai-code-reviewer` (`<DAI_REPO_ROOT>/.claude/skills/dai-code-reviewer/SKILL.md`): a read-only pre-completion code review for implementation slices through a productized, production-minded, minimal-design lens. Review dimensions: correctness; syntax/build safety; idiomatic C#/.NET, Python/FastAPI, Angular/TS; simplicity; naming/casing; comments (lowercase ascii); architecture cohesion (factory model + tenant/source/tool/agent boundaries; no niche logic in platform core; thin frontend / backend-owned orchestration); productized lens; testing; graphify usage (navigation/blast-radius only, verify against source); security/safety; completion discipline (no approval on skipped tests/unsupported claims; honest partial over inflated). Fixed output: DAI CODE REVIEW (scope / findings / verification / recommendation = approve | approve with notes | request changes). Inspected existing skills first (rule: no duplication): `dai-test-discipline` (which tests to run), `dai-typescript-angular-quality` (TS/Angular writing rules), `dai-grill-with-vault`, `dai-agent-handoff`, `dai-docs-architect` -- none perform a holistic pre-completion review, so created new. Registered in `dai-skill-router` (routing rule: any code-changing slice -> review before final handoff; + pair-with). Updated the vault skills inventory (skill count 11 -> 12). Separately committed a pending `<DAI_REPO_ROOT>/CLAUDE.md` change adding operating directives (skills gate / graphify orientation / verify against source-tests-vault / code review before completion / structured handoffs). What did NOT change: no application source, no runtime behavior, no semantic/cloud/LLM extraction, no graphify run, no `graphify-out/`, `.claude/settings.json` untouched. Repo states: `<DAI_REPO_ROOT>` main 8a77a79 before; `<DAI_VAULT_ROOT>` main 668ee5b before. Committed both repos (docs(skills): add dai code review workflow), not pushed. Next: Agent Slice Workflow Doctrine v1 -- first P1 backlog doc, now better supported by the docs + review skills.

---

status: Agent Slice Workflow Doctrine v1 complete 2026-06-23 -- docs/vault only, no code/runtime change. Wrote the canonical slice-lifecycle doctrine at `06 Execution/agent-slice-workflow-doctrine-v1.md` using the dai-docs-architect 21-section template: a DAI slice is the unit of controlled platform change with boundary/purpose/constraints/verification path/handoff; three phases (orient -> execute -> verify-and-hand-off); gates = Skills Gate (`dai-skill-router`) at start, `dai-grill-with-vault` for architecture-heavy work, `dai-docs-architect` for docs, `dai-test-discipline` for tests, `dai-code-reviewer` before completion on code-changing slices, `dai-agent-handoff` at end; truth hierarchy = runtime+tests > source > contracts/vault > handoffs > graphs/assumptions (navigation only); no domain-mixing unless scoped; honest partial over inflated completion; do not commit generated/local/machine files; `dai-slice-runner` explicitly deferred to encode (not invent) this doctrine. Skills Gate run: `dai-docs-architect` is not natively loadable in this workspace-root session (it is a project skill under `<DAI_REPO_ROOT>/.claude/skills`), so its template was applied from the committed SKILL.md and recorded as expected-but-missing with fallback. Graphify not used (small, known file set; direct reads sufficed). Linked from `documentation-doctrine-backlog-v1.md` (item 8 marked DONE; top-3 re-ranked) and `dai-skills-inventory-v1.md` (Skills Gate section). What did NOT change: no application source, no runtime behavior, no skills changed, no semantic/cloud/LLM extraction, no graphify run, no `graphify-out/`, no `dai-slice-runner` created. Repo states: `<DAI_REPO_ROOT>` main 6368c66 (untouched this slice); `<DAI_VAULT_ROOT>` main 2a0262b before. Committed `<DAI_VAULT_ROOT>` only (docs(execution): document agent slice workflow doctrine), not pushed. Next: Source Depth and Evidence Sufficiency Doctrine v1 (next P1); defer DAI Slice Runner Skill v1 until another doctrine slice exercises this lifecycle.

---

status: Source Depth and Evidence Sufficiency Doctrine v1 complete 2026-06-23 -- docs/vault only, no code/runtime change. Wrote the canonical anchor at `04 Products/sports-v1/source-depth-and-evidence-sufficiency-doctrine-v1.md` (dai-docs-architect 21-section template) reconciling five implemented docs into one doctrine: two-axis model = confidence (internal coherence, numeric, relative, preserved) vs evidence sufficiency (independent grounding, absolute from `evidenceRichness`, tiered thin<=1/moderate==2/rich>=3); source support splits into breadth (`SourceSufficiency` groups + Critical/Supporting/Contextual tiers + sport-critical group) and depth (`SourceDepth` levels none/identity_only/shallow/enriched), independent of each other; buyer-advertised strength derives from BOTH and thin evidence caps High->Medium (`gateAdvertisedStrength` in buyer-signal-summary.ts, humility reason `advertised_strength_limited_by_evidence`, never mutates confidence/lean/posture). Captures the named failure mode (relative completeness masking absolute scarcity; MLB maxGrounded==1 -> 1-of-1 looks complete -> high confidence on thin evidence), buyer trust as protected stock, provisionality (numbers provisional until Outcome Reconciliation), and buyer copy posture (allowed direction language; no tout posture; no internal gap terms per buyer-copy-safety-v1). Confidence != evidence sufficiency; evidence sufficiency guides copy posture only, never mutates facts. Skills Gate run; dai-docs-architect applied from committed SKILL.md (not natively loadable in this workspace-root session), recorded as expected-but-missing with fallback. Graphify NOT used -- vault docs cite exact source paths/fields (evidenceRichness, SourceDepth, SourceSufficiency, SportsEvaluator.cs, buyer-signal-summary.ts); direct reads were authoritative. Placement justified: 04 Products/sports-v1 co-locates it with the five anchored docs and matches convention; abstract two-axis model flagged for promotion to 02 Platform when a second niche needs it (deferred). Linked from documentation-doctrine-backlog-v1.md (item 3 marked DONE; top-3 re-ranked: Tool Gateway #1, Cognitive Protocol Anchor #2, Tenant #3). What did NOT change: no application source, no runtime behavior, no skills changed, no semantic/cloud extraction, no graphify run, no graphify-out/, no dai-slice-runner. Repo states: `<DAI_REPO_ROOT>` main 6368c66 (untouched); `<DAI_VAULT_ROOT>` main db1738e before. Committed `<DAI_VAULT_ROOT>` only (docs(product): document source depth evidence doctrine), not pushed. Next: Tool Gateway and Agent Permissions Doctrine v1 (next P1).

---

status: Tool Gateway and Agent Permissions Doctrine v1 complete 2026-06-23 -- docs/vault only, no code/runtime change. Wrote the canonical anchor at `02 Platform/architecture/tool-gateway-and-agent-permissions-doctrine-v1.md` (dai-docs-architect 21-section template) over the charter + station blueprint + probe-refresh readiness. Doctrine: tools are shared platform infrastructure reached only through `ToolGateway.InvokeAsync`, which fails closed on registry miss or `IProtocolToolAccessPolicy.IsAllowed(definition, callerNode)` deny (no handler resolves, no transport call); permission is declared config (`ToolDefinition.AllowedProtocolNodes`), never the model's ask; stations cannot widen their own permissions; every invocation emits one `ToolGatewayInvocation` log (tool/node/run/tenant/correlation/cost/outcome/duration). Current enforcement is stage-level (`platform.reference/retrieve/analyze`); per-station, cost-class, idempotency, and tenant-tier enforcement are DEFERRED (honest current-vs-future split). Retrieve-only is the default; mutation requires an audit envelope. Dormant probe-refresh chain stays default-off (Enabled/AllowGatewayExecution/PersistAuditRecord/AllowArtifact|Confidence|Posture|LeanMutation all false); merge audit is idempotent, tenant-scoped, read-only; protected fields (confidence/posture/lean/artifact version/tenant/run id/raw-signal overwrite/audit deletion) are forbidden changes; gateway execution, if ever allowed, constrained to `platform.retrieve`. Cross-tenant access forbidden; tenant derives from caller identity; reads return NotFound rather than leaking existence. Skills Gate run; dai-docs-architect applied from committed SKILL.md (not natively loadable in this workspace-root session), recorded as expected-but-missing with fallback. Graphify USED for orientation (explain ToolGateway/ProbeRefreshExecutor, query ProtocolToolAccessPolicy) to locate symbols, then verified directly against source: read `Tools/ToolGateway.cs`, `IToolGateway.cs`, `ToolDefinition.cs`, `ToolTelemetry.cs` and confirmed fail-closed + `IProtocolToolAccessPolicy`/`ProtocolToolAccessPolicy.Default` + `ProtocolToolAccessPolicyTests` exist. Placement justified: 02 Platform/architecture top-level (cross-cutting platform governance over all tools; sits beside security-and-permissions.md and orchestration.md; not cognitive-factory-specific). Linked from documentation-doctrine-backlog-v1.md (item 6 marked DONE; all three P1 items now done; top-3 re-ranked to P2 anchors). What did NOT change: no application source, no runtime behavior, no skills changed, no semantic/cloud extraction, no graphify-out/ committed, no dai-slice-runner. Repo states: `<DAI_REPO_ROOT>` main 6368c66 (untouched); `<DAI_VAULT_ROOT>` main 6fe1ad1 before (1 unpushed at slice start). Committed `<DAI_VAULT_ROOT>` only (docs(platform): document tool gateway permissions doctrine), not pushed. Next: DAI Slice Runner Skill v1 (P1 doctrine set complete -> encode the proven slice-workflow lifecycle), or Cognitive Protocol Doctrine Anchor v1 (P2) if more doc consolidation is preferred.

---

status: DAI Slice Runner Skill v1 complete 2026-06-23 -- skills + docs only, no code/runtime change. Created `dai-slice-runner` (`<DAI_REPO_ROOT>/.claude/skills/dai-slice-runner/SKILL.md`): the executable encoding of `06 Execution/agent-slice-workflow-doctrine-v1.md`. 8-step lifecycle (orient -> bound the slice -> select skills/Skills Gate -> execute narrowly -> verify -> review -> handoff -> commit/push discipline) over the doctrine's truth hierarchy (runtime+tests > source > contracts/vault > handoffs > graphs/assumptions); pairs docs->dai-docs-architect, code->dai-code-reviewer, tests->dai-test-discipline, architecture->dai-grill-with-vault, handoff->dai-agent-handoff, verbosity->dai-token-tight. Requires two fixed outputs: `DAI SLICE START` (slice/starting state/skills selected/orientation/verification plan) and `DAI SLICE CLOSE` (scope completed/verification/repo state/handoff). It coordinates the existing skills, does not replace judgment, and when it and the doctrine diverge the doctrine wins. Graphify: navigation/blast-radius only, never on dai-vault, never semantic. Confirmed no prior runner skill existed (13 total now). Registered in `dai-skill-router` (routing rule + pair-with). Updated vault skills inventory (12 -> 13). Skills Gate run; dai-write-skill applied from committed SKILL.md (not natively loadable in this workspace-root session), recorded as expected-but-missing with fallback. Graphify NOT used (skill-authoring over already-read skill/doc files; no code blast-radius). What did NOT change: no application source, no runtime behavior, no semantic/cloud extraction, no graphify run, no graphify-out/, no automation/hooks/CI. Repo states: `<DAI_REPO_ROOT>` main 6368c66 before; `<DAI_VAULT_ROOT>` main e138147 before (2 unpushed at slice start: 6fe1ad1 + e138147). Committed both repos (docs(skills): add dai slice runner workflow), not pushed. Next: a first live implementation slice driven by dai-slice-runner + dai-code-reviewer to exercise the runner end to end, or Cognitive Protocol Doctrine Anchor v1 (P2).

---

status: Advertised-Strength Server Authority v1 complete 2026-06-23 -- IMPLEMENTED + verified (TDD). FIRST live implementation slice run through dai-slice-runner + dai-code-reviewer + dai-test-discipline + Graphify. Lifted buyer-advertised strength (confidence band AFTER the evidence-sufficiency humility cap) from a frontend-only concern (`gateAdvertisedStrength` in buyer-signal-summary.ts) into the artifact read-model contract so non-angular consumers inherit the cap. New pure `DevCore.Api/AgentRuns/AdvertisedStrength.cs`: `AdvertisedStrengthDeriver.Derive(confidence, evidenceRichness)` -> `AdvertisedStrength(Band, RawBand, Reason)`; reproduces the existing buyer thresholds (0.70/0.45) so the buyer-visible band is UNCHANGED, caps High+thin(<=1)->Medium with reason `advertised_strength_limited_by_evidence`, fails open on null richness, never raises a band, never mutates confidence/lean/posture. `AgentRunArtifactDto` gains a trailing optional `AdvertisedStrength`; `AgentRunsController.GetArtifact` derives it read-only beside SourceSufficiency from `artifact.Confidence` + `artifact.EvidenceRichness` (Confidence passed through unchanged). Investigation result: authority was frontend-only; backend exposed raw confidence+evidenceRichness+SourceSufficiency but no advertised strength; decision = add smallest server-owned read-model projection, defer frontend adoption. Graphify USED for orientation only (query gateAdvertisedStrength/advertisedStrength -> located buyer-signal-summary.ts + AgentRunArtifactDto), verified directly in source; no graph finding relied on as authority. TDD: red (missing type compile fail) -> green; 13 new pure tests; AgentRunsControllerTests 54/54; declared final verification full DevCore.Api.Tests 884/884. dai-code-reviewer: approve with notes (non-blocking: no /artifact endpoint integration test; frontend+backend now compute the same band independently = divergence risk until frontend adopts the server value). What did NOT change: no FastAPI/model/SportsEvaluator/numeric confidence, no lean/posture, no buyer copy, no Angular (frontend untouched), no schema/migration, no Tool Gateway/source/probe/reconciliation/billing/auth/tenant. Repo states: `<DAI_REPO_ROOT>` main 77ad95a before -> code commit a499b80 (feat(sports): derive advertised strength server-side); `<DAI_VAULT_ROOT>` main 4d0f10c before -> docs commit (this entry + 04 Products/sports-v1/advertised-strength-server-authority-v1.md). Not pushed. Next: Advertised-Strength Frontend Adoption v1 (render artifact.advertisedStrength, retire the duplicate frontend computation, add the endpoint test), or Cognitive Protocol Doctrine Anchor v1 (P2).

---

status: Reconciliation and Fresh Batch Validation v1 complete 2026-06-23 -- operational validation, NO code change. Ran through dai-slice-runner + dai-test-discipline (service/endpoint checks); no dai-code-reviewer (no code changed). Brought up the full local stack (Docker Desktop launched; devcore-sql :1433 ready; FastAPI :8000 ok; DevCore.Api :5007 health 200, built from a499b80 so advertised-strength derive-on-read is live; Angular not started). Reconciliation inventory (GET /recent + GET /{id}/evaluation): 25 recent MLB runs, 6 already reconciled, 19 not; RECONCILED THIS SLICE = 0 -- no authoritative outcomes available and the harness/rule never guesses; several of the 19 are duplicate test-matchup reruns (e.g. Yankees@Tigers x6) needing exclusion before reconcile to avoid MultipleMatches. Fresh batch: 4 distinct 2026-06-23 MLB games via POST /api/agent-runs, all completed -- 6a2d433e Mariners@Pirates (conf 0.75, rich 2/moderate, advStrength High), 702d433e RedSox@Rockies (0.80, 2, High), 722d433e Orioles@Angels (0.70, 2, High), 752d433e Braves@Padres (0.45, 1/thin, Medium). Advertised-strength spot check on live /artifact: field PRESENT on all (derive-on-read works); confidence preserved; moderate+high correctly stays High (not over-capped); thin+non-high (run 4) stays Medium; thin+High cap NOT observed live (today's slate grounds 2 signals=moderate) but covered by 13/13 unit tests + the documented historical case; frontend adoption NOT performed (intentional). ISSUE FOUND: run 722d433e prose/summary lean Orioles (away) but leanSide=home -- ArtifactDirectionConsistency guard CORRECTLY flagged PotentialMismatch; leanSide-derivation discrepancy worth a follow-up; does not affect advertised strength; not fixed (rule 11). Report: 04 Products/sports-v1/validation/reconciliation-and-fresh-batch-validation-2026-06-23-v1.md. What did NOT change: no application source, no buyer copy, no advertised-strength derivation, no confidence/lean/posture, no Tool Gateway/source/probe/reconciliation logic, no billing/auth/tenant; only 4 new dev agent-run rows. Repo states: `<DAI_REPO_ROOT>` main a499b80 (clean, untouched, 1 unpushed preserved); `<DAI_VAULT_ROOT>` main 51b0f75 before -> docs commit (this entry + the validation report). Not pushed. Next: Advertised-Strength Frontend Adoption v1; plus follow-ups -- investigate the 722d433e leanSide mismatch, and a Reconciliation Results-Input + Dedup slice once authoritative results exist.

---

status: LeanSide Direction Consistency Fix v1 complete 2026-06-23 -- IMPLEMENTED + verified (TDD), code + docs. Ran through dai-slice-runner + dai-test-discipline + dai-code-reviewer; Graphify for orientation only (graph stale for LeanSide -> verified in source). ROOT CAUSE of run 722d433e (Orioles@Angels, leanSide=home vs prose-lean Orioles/away): model self-inconsistency -- FastAPI analyzer has the model emit both `lean` prose and structured `lean_side`; the prompt instructs agreement but does not enforce it; `_parse_response` (no team names) takes lean_side verbatim; SportsComposer passes it through. NOT a derivation/mapping/parsing bug or guard false positive (category 1, model/prose contradiction). FIX (deterministic containment, read-only): new `ArtifactDirectionContainment.PresentedLeanSide` next to the existing guard -- when ArtifactDirectionConsistency=PotentialMismatch, the artifact read projection suppresses structured leanSide to null (the mismatch stays visible via the Status); Consistent/Ambiguous/NotEvaluable preserve the side. Wired into AgentRunsController.GetArtifact. Persisted artifact + denormalized AgentRun.LeanSide (reconciliation matcher) + confidence + advertised strength + posture + buyer copy all UNCHANGED; contains both the existing run and future runs at the contract surface; no new direction system. TDD red->green: 8 new tests (incl. the Orioles@Angels regression + a consistent control); adjacent suites 71; declared final verification full DevCore.Api.Tests 891/891. dai-code-reviewer: approve with notes (non-blocking: persisted AgentRun.LeanSide still feeds the matcher as home for genuinely-mismatched runs -- deeper compose/persist containment deferred to honor rule 12; model can still emit the contradiction -- prompt/parser hardening deferred). LIVE re-verify (had to stop+restart the API to clear the build file-lock): 722d433e now projects leanSide=null + PotentialMismatch (confidence 0.7, advStrength High unchanged); control 6a2d433e keeps leanSide=away + Consistent. Run 722d433e: contained at the contract; recommend excluding it from calibration (dev/test run). Advertised-Strength Frontend Adoption CAN resume (no blocker; advertised strength never affected). Doc: 04 Products/sports-v1/lean-direction-consistency-review-v1.md. What did NOT change: no FastAPI prompt/parser/model, no compose persist logic, no confidence/advertised-strength/posture, no buyer copy, no Tool Gateway/source/probe/reconciliation/billing/auth/tenant. Repo states: `<DAI_REPO_ROOT>` main a499b80 before -> code commit e4a00de (fix(sports): enforce lean direction consistency); `<DAI_VAULT_ROOT>` main d974ce2 before -> docs commit (this entry + the review doc). Not pushed. Next: Advertised-Strength Frontend Adoption v1.

---

status: Advertised-Strength Frontend Adoption v1 complete 2026-06-24 -- IMPLEMENTED + verified (TDD), Angular frontend + tests. Ran through dai-slice-runner + dai-skill-router (Skills Gate) + dai-test-discipline + dai-code-reviewer; Graphify refreshed (`graphify update .`) and used for orientation only. Closes the dual authority opened by Advertised-Strength Server Authority v1 (a499b80): the Angular buyer path now RENDERS the server-owned `advertisedStrength` and treats its own band derivation as a legacy-only compatibility fallback. Investigation result: the frontend model did NOT expose the server field; `buildBuyerSignalSummary` ignored it and re-derived an identical band via `gateAdvertisedStrength`; preferring the server value with a legacy fallback is safe and typed. Changes (3 files, all `apps/sports-app`): (1) `core/models/agent-run.model.ts` -- new optional `AdvertisedStrengthDto` (band/rawBand/reason) and `AgentRunArtifactDto.advertisedStrength?`; (2) `analyzer/buyer-signal-summary.ts` -- `serverAdvertisedStrength(artifact)` PRIMARY AUTHORITY reads the server band/reason (band narrowed via `asBuyerBand` so an unexpected token never leaks), `fallbackAdvertisedStrength(artifact)` FALLBACK ONLY wraps the prior confidenceBand+gateAdvertisedStrength derivation now clearly demoted, and `buildBuyerSignalSummary` chooses `serverAdvertisedStrength(artifact) ?? fallbackAdvertisedStrength(artifact)`; (3) `analyzer/buyer-signal-summary.spec.ts` (+6 tests). Buyer-visible band UNCHANGED for equivalent inputs (server reproduces the same 0.70/0.45 + High+thin->Medium derivation as the fallback). Confidence/lean/posture/buyer copy UNCHANGED; the existing no-mutation test stays green. TDD red->green: scoped `npm test --include="**/buyer-signal-summary.spec.ts"` 29/29 (was 23); declared final verification full sports-app suite 65/65 + `npx ng build` clean; backend not run (no backend change). Tests prove: server field present -> frontend uses it (server Medium beats a local-uncapped High); server field missing -> fallback preserves legacy behavior; High+thin does not render High on the fallback path; null/unexpected server band -> null (defensive). dai-code-reviewer: approve (no blocking; non-blocking: `rawBand` typed but unconsumed = intentional contract parity). Graphify USED for orientation only (query AdvertisedStrength / AgentRunArtifactDto -> located the cross-stack contract link), verified directly in source; no graph finding relied on as authority. LeanSide Direction Consistency Fix v1 (e4a00de) INTENTIONALLY UNCHANGED -- advertised strength is confidence/evidence-derived and was never affected by structured-leanSide containment (verified in source + lean-direction-consistency-review-v1.md); no model-alignment compile/type issue arose. What did NOT change: no backend advertised-strength derivation (`AdvertisedStrength.cs` untouched), no FastAPI/model, no confidence/lean/posture, no buyer copy, no Tool Gateway/source/probe/reconciliation/billing/auth/tenant, no semantic/cloud graphify, no graphify-out/ committed. Repo states: `<DAI_REPO_ROOT>` main e4a00de before (1 unpushed preserved) -> code commit (feat(sports-app): render server advertised strength); `<DAI_VAULT_ROOT>` main ef71aef before (1 unpushed preserved) -> docs commit (this entry + 04 Products/sports-v1/advertised-strength-frontend-adoption-v1.md). Not pushed. Next: optional dev-only surfacing of the `advertised_strength_limited_by_evidence` humility reason on the artifact inspection page (not buyer-facing).

---

status: Directional-Contrast Cohort Settlement-Completion Pass v1 complete 2026-06-24 -- OPERATIONAL settlement, no code change. Ran through dai-slice-runner + dai-skill-router gate + superpowers:verification-before-completion. Brought up the stack (Docker Desktop -> devcore-sql :1433 Up; DevCore.Api :5007 health 200 at HEAD 1a7c84c; MLB StatsAPI reachable HTTP 200; FastAPI not needed). Re-verified live (audit not trusted) and found discrepancies: only 2 of the recent-25 were reconciled (not 6 -- the rest aged out of the window; whole-corpus pre-state was the expected 35/35 by sqlcmd); the duplicate landscape was larger than audited (5 true Yankees@Tigers dupes not 6 -- the 6th, a6b5433e, IS cohort run 200013 -- plus a Guardians@WhiteSox dupe e5b5433e and a Cubs@Mets dupe d8b5433e the audit missed); and cohort game 200018 (Cubs@Mets, gamePk 823613) is POSTPONED, so only 9 of 10 cohort runs were settle-eligible. Exclusion pass (8 via POST /{id}/exclude): 7 diagnostic (d2b5433e dab5433e dbb5433e deb5433e e2b5433e Yankees dupes + e5b5433e Guardians dupe + d8b5433e Cubs dupe), 1 excluded (722d433e, documented LeanSide/prose contradiction; persisted LeanSide=home, read contained to null). Settlement pass (12 outcomes via POST /{id}/outcome, scores from MLB StatsAPI schedule+linescore, source=statsapi, never guessed; each evaluation read back via GET /{id}/evaluation): cohort 9 settled (correct 2 b9b/d1b, incorrect 5 a6b/abb/b2b/c7b/cab, inconclusive 2 b8b/c0b), fresh batch 3 settled (correct 2 6a2/702, inconclusive 1 752). Platform evaluations matched hand-grading exactly. Corpus 35/35 -> 47/47 (sqlcmd pre+post): correct 20->24, incorrect 11->16, inconclusive 4->7, decided 31->40, hit rate 64.5% -> 60.0%. DIRECTIONAL FINDING: the balanced cohort scored 2/7 decided (28.6%), BELOW the ~63% home base rate -- home leans 2/5 (40%), away leans 0/2 (0%), on a road-heavy slate (5 of 7 decided games were away wins). Directional discrimination remains UNPROVEN; confidence flat 0.75 cannot discriminate within-cohort and was over-confident on this slate (no basis to raise it). Calibration STILL STALLED; do not over-interpret (n=7 decided, single slate). Assessment: another balanced cohort needed; settle 200018 after replay; Reconciled-Outcome Calibration Read v2 now warranted (corpus has its first balanced directional data; keep the cohort as its own regime, never pool). Excluded-run total 3 -> 11. What did NOT change: no application source, no model/prompt/FastAPI, no schema, no confidence/lean/posture logic, no advertised-strength derivation, no reconciliation-matcher logic, no buyer copy, no Tool Gateway/source/probe/billing/auth/tenant; operational rows only (12 outcome+eval, 8 exclusion flags). Doc: 04 Products/sports-v1/calibration/directional-contrast-cohort-settlement-completion-v1.md. Repo states: dai main 1a7c84c (untouched, synced); dai-vault main 6e8c1b2 before -> docs commit (this entry + the settlement doc). Not pushed. Next: Reconciled-Outcome Calibration Read v2.

---

status: Reconciled-Outcome Calibration Read v2 complete 2026-06-24 -- READ-ONLY analysis, no code/runtime change. Ran through dai-slice-runner + dai-skill-router gate + dai-grill-with-vault + superpowers:verification-before-completion. Read-only sqlcmd over the 47-run settled corpus (devcore-sql); regimes assigned by AgentRunKey, never pooled. OVERALL: 47 settled / 40 decided / 24 correct / 16 incorrect / 7 inconclusive / 60.0% hit (down from 64.5% at 35). Home won 23/40 (57.5%) and the system leaned home 33/40 (82.5%); an "always lean home" baseline scores 57.5%, so the system's 60.0% is within binomial noise (n=40). REGIME (kept separate): pre-market thin 9/11 (81.8%), enriched 190x 6/6 (100%), fresh 210x 2/2 (100%), v2 legacy 2/3, NBA 1/3 (33%), identity-only 180x 2/8 (25%), directional 200x 2/7 (28.6%). NO advantage survives slate scrutiny: 190x and 200x are BOTH enriched starter-depth yet 100% vs 29% -- 190x was all-home leans on an all-home-win slate, 200x was a balanced cohort on a road-heavy slate. Enriched depth did NOT replicate; the gains are slate-selection effects, not grounding. CONFIDENCE: clustered at 0.75 (34/47); every decided run is 0.675-0.80; sub-0.65 runs are all null-lean inconclusive. Confidence does not vary among decided runs -> no ranking information, not calibrated. 0.80 n=1 (can't compare). LEAN: home 20/33 (60.6%) vs home base rate 57.5% (no edge); away 4/7 (57.1%, n too small, bimodal: cohort away 0/2, fresh away 2/2); null 7 inconclusive. The system is effectively leaning home and home accuracy reflects the base rate. SOURCE DEPTH: thin 78.6% > enriched 61.5% > identity-only 25%; richness 1 -> 78.6% vs richness 2 -> 50.0% (inverted); deeper grounding not shown to help; enriched bimodal by slate. HEALTH: STALLED (the balanced test showed no edge; overall fell toward base rate; not regressing functionally, just measured more honestly). DECISION: (1) no evidence of directional discrimination; (2) confidence not calibrated (near-constant); (3) another balanced cohort REQUIRED (decisive evidence only n=7 decided); (4) tuning NOT justified (no validated target; would fit slate noise; rule 9 + Confidence Calibration Rules v1); (5) highest-value next experiment = larger Directional-Contrast Cohort Capture v2 balanced across home/away leans AND across multiple game-days/slates, graded on structured LeanSide, PAIRED with a market-baseline comparison (does the lean beat the closing-line favorite?) so the test is signal-vs-market not only signal-vs-home-base-rate. HONEST CONCLUSION: insufficient evidence to confirm signal, trending toward evidence AGAINST a directional edge; the corpus shows no measurable edge (accuracy = base rate, confidence non-discriminating, depth non-improving) but the only balanced subset is too small to be conclusive. Path: gather more data via one targeted balanced experiment -- not continue-as-is, not tune. dai-grill-with-vault: every v1 finding holds and is strengthened; no contradiction with doctrine. What did NOT change: no application source, no model/prompt/FastAPI, no schema, no confidence/lean/posture/source-depth logic, no advertised strength, no buyer copy, no Tool Gateway/source/probe/reconciliation/billing/auth/tenant; zero DB writes (totals stay 47/47). Doc: 04 Products/sports-v1/calibration/reconciled-outcome-calibration-read-v2.md. Repo states: dai main 1a7c84c (untouched, synced); dai-vault main 5b354f5 before -> docs commit (this entry + the v2 read). Not pushed. Next: Directional-Contrast Cohort Capture v2 (larger, multi-slate, balanced) + market-baseline comparison.

---

status: Directional-Contrast Cohort Capture v2 + Market Baseline Capture v1 complete 2026-06-24 -- EXPERIMENT CAPTURE, no tuning/code/prompt/confidence/depth/advertised-strength/buyer change. Ran through dai-slice-runner + dai-skill-router gate + superpowers:verification-before-completion. Brought up the stack (Docker/devcore-sql Up; DevCore.Api :5007 200 at HEAD 1a7c84c; FastAPI :8000 /api/ping 200; the-odds-api.com reachable; MLB StatsAPI reachable). Generated today's full MLB slate (15 POST /api/agent-runs, RunType sports.matchup.analysis, gameDate 2026-06-24); 14 completed identity-bearing (mlb_statsapi + gamePk), 1 (Athletics@Giants, late west-coast game) hung pending with no artifact -> excluded invalid. Captured an external MARKET BASELINE per game: de-vigged consensus moneyline from the-odds-api.com (~9 books/game, h2h) -> favorite side + implied probability, recorded alongside each run. COHORT SUMMARY (14): lean home 8 / away 5 / null 1 (far more balanced than the historical 82% home); confidence avg 0.764 (10x0.75 + 4x0.80, still tightly clustered); advertised strength High x14; evidence richness 2 x14; source sufficiency moderate x14; market favorite home 7 / away 7 (balanced slate). HEADLINE OBSERVATION (one slate, do not over-interpret): DAI's lean TRACKS THE MARKET FAVORITE on 12 of 13 decided-lean games (92%); lone disagreement Brewers@Reds (DAI home/Reds vs market away/Brewers); DAI abstained null on Braves@Padres (market away). This reframes the Read v2 "82% home" finding -- on a market-balanced slate DAI went 8h/5a, so the prior home skew looks like MARKET-FOLLOWING on home-favorite-heavy historical slates, not an intrinsic home bias; DAI's directional signal appears largely market-derived, which structurally limits its ability to beat the market (edge must come from disagreements + null abstentions + confidence). MARKET COMPARISON READINESS verified: every field recorded (DAI lean, confidence, advertised strength, gamePk, market favorite, implied prob) so Read v3 can compute DAI accuracy, market accuracy, and DAI-minus-market delta after settlement. IDENTITY: all 14 completed, identity-bearing, active, v3, reconcilable per-run on the persisted LeanSide. NOTE gamePk 823613 collision: today's Cubs@Mets run d879433e shares gamePk 823613 with the postponed directional-cohort run 200018 (beb5433e, away lean) -- same rescheduled game, OPPOSITE leans; reconcile both per-run; settling 823613 closes the directional cohort's last open game. SETTLEMENT TIMING: games settle ~2026-06-25T04:00Z+; reconcile 2026-06-25. COHORT ADEQUACY: NOT YET ADEQUATE -- 14 runs, 13 decided leans, 5 away, only 1 market disagreement, ONE slate; Read v2's ~20-30-decided-away-lean multi-slate target unmet (upcoming endpoint exposes only today's slate). RECOMMENDATION (no tuning): (1) reconcile 2026-06-25 on authoritative StatsAPI scores; (2) capture 2-3 more daily slates (06-25/-26) before any directional/market conclusion -- DAI~market agreement makes disagreement samples the scarce essential data; (3) Read v3 ready to DESIGN, run only after >=3 settled slates pooled; do NOT tune (no validated discrimination target). What did NOT change: no prompt/model/FastAPI logic, no confidence/depth/advertised-strength/lean/buyer, no Tool Gateway/reconciliation/billing/auth/tenant, no schema; only 14 new agent-run rows + 1 excluded + an external market baseline. Settled corpus totals unchanged 47/47 (no outcomes posted; games unsettled; no scores guessed). Doc: 04 Products/sports-v1/calibration/directional-contrast-cohort-capture-v2.md. Repo states: dai main 1a7c84c (untouched, synced); dai-vault main 89b24a0 before -> docs commit (this entry + the capture doc). Not pushed. Next: settle this cohort 2026-06-25, capture slates 2-3, then Reconciled-Outcome Calibration Read v3 (DAI vs market vs outcome).

---

status: DAI Slice Prompt Architect Skill v1 complete 2026-06-24 -- skills + docs only, no runtime/code/prompt/confidence/depth/advertised-strength/Tool-Gateway/reconciliation/billing/auth/tenant/buyer change. Ran through dai-slice-runner + dai-skill-router gate + dai-write-skill + dai-docs-architect + superpowers:verification-before-completion. Created `dai/.claude/skills/dai-slice-prompt-architect/SKILL.md` (15 sections): a doctrine-backed prompt compiler that converts VERIFIED project state (handoffs, current-slice, vault doctrine, skills inventory, calibration/backlog/repo state, Graphify orientation) into a reviewable, executable next-slice prompt. It PROPOSES; the runner EXECUTES; the user/architect APPROVES. 6 internal modes: next-slice-prompt, prompt-review, prompt-learning-note, skill-improvement-proposal (proposes only; >=2 instances or 1 severe failure; overfitting risk stated; never applies), next-slice-file (canonical `06 Execution/prompting/next-slice.md` + archive), slice-type-specialization (11 slice types with required skills/risks/verification/non-goals/Graphify/handoff emphasis). 15-criterion prompt quality rubric (pass / needs tightening / unsafe-revise). Explicit learning loop (draft -> execute -> handoff -> review -> lesson note -> propose -> human approve -> separate skill slice). HARD BOUNDARY: never self-modifies, never silently updates itself or other skills; no automation/scripts/hooks/CI; Graphify navigation-only, never on dai-vault. Verified NO existing skill covered next-slice prompt generation (closest: slice-runner executes, agent-handoff summarizes, skill-router routes -- none compile a next prompt); reuse-vs-create decision = create, no duplication. Created vault prompting area `06 Execution/prompting/`: slice-prompt-architecture-doctrine-v1.md (canonical role/runner/handoff/Graphify/vault/approval relationships, anti-goals, learning loop, next-slice semantics, truth hierarchy; consolidates the proposed prompts/ + prompting/ locations into ONE home to prevent drift), prompt-patterns-and-lessons-v1.md (13 seeded lessons incl. do-not-tune-before-measurement, Graphify-navigation-only, keep-regimes-separate, exclude-contaminated-runs, capture-market-baseline, explicit-non-goals, proposal-until-validated), next-slice.md (proposal: Directional-Contrast Multi-Slate Settlement + Capture Continuation v1 -- settle the 14-run cohort-v2 on authoritative StatsAPI finals + capture 1-2 more slates toward Read v3; with header block, full prompt draft, and a runner validation checklist; clearly marked PROPOSAL not authority), archive/README.md (archived prompts are historical, never executed). Updated dai-skill-router (routing rule + pair-with) and the vault skills inventory dai-skills-inventory-v1.md (full entry + skill-layer update note; count 13 -> 14). Verification: git clean before; only intended files changed; NO application/runtime source touched (platform/apps/services untouched); no graphify-out tracked; new vault docs cross-link coherently; next-slice.md marked proposal; skill states no-self-modification. What did NOT change: no runtime code, no product/model prompts, no confidence/source-depth/advertised-strength/lean/buyer copy, no Tool Gateway/reconciliation/billing/auth/tenant, no automation/hooks/CI, no settled calibration corpus (still 47/47). Repo states: dai main 1a7c84c before -> code/skills commit (docs(skills): add dai slice prompt architect); dai-vault main 5c9ff79 before -> docs commit (docs(prompting): add slice prompt architecture loop). Not pushed. Next recommended slice: the proposed Directional-Contrast Multi-Slate Settlement + Capture Continuation v1 (see 06 Execution/prompting/next-slice.md), validated by dai-slice-runner against live repo state first.

---

status: DAI Slice Prompt Architect Enhancement v1 complete 2026-06-24 -- skills + docs only, no runtime/code/prompt/confidence/behavior change. Ran through dai-slice-runner + dai-skill-router gate + dai-write-skill + superpowers:verification-before-completion. Added a Strategic Snapshot + Opportunity Cost step to dai-slice-prompt-architect (new section 16; no new skill -- count stays 14). MODE 1 now SILENTLY evaluates seven inputs (WhyNow, FactoryOrProduct, RevenuePath, DecisionScarcity, StrategicHorizon, Invalidation, OpportunityCost) as reasoning inputs and compresses them into a 5-7 line STRATEGIC SNAPSHOT (Type/Horizon/Why Now/Scarce Signal/Revenue Path/Primary Risk) + a one-line OPPORTUNITY COST ("Chosen over <real alt> because <deciding factor>"), inserted immediately after the recommendation rationale and before the prompt body. Token discipline enforced: target < 60 added tokens, phrases over paragraphs, drop obvious fields -- the enhancement raises decision quality, not prompt length. Decision-scarcity guidance: prefer slices that resolve scarce information (DAI-vs-market disagreements, confidence outliers, high-confidence misses, low-evidence wins, reversals, sparse cohorts) over volume. Invalidation folds into Primary Risk (dedicated note only when risk is unusually high). Authority boundary reaffirmed: the snapshot informs PRIORITIZATION only, grants no execution authority -- architect proposes, runner validates live state, user is final approval. Updated MODE 1 output-format line to reference the snapshot. Retrofitted the live next-slice.md to the new format (Factory / Immediate / settlement-perishable / scarce=DAI-vs-market disagreements + away leans / Indirect revenue / risk=small discriminating sample; Opportunity Cost: chosen over Calibration Read v3 because settlement data is time-sensitive and <3 pooled slates exist). Added one reusable pattern/lesson to prompt-patterns-and-lessons-v1.md and a skill-layer note to the inventory. What did NOT change: no runtime code, no product/model prompts, no confidence/source-depth/advertised-strength/lean/buyer copy, no Tool Gateway/reconciliation/billing/auth/tenant, no automation/hooks/CI, no settled corpus (47/47); the skill remains non-self-modifying and proposal-only. Repo states: dai main bf77360 before -> skill commit (docs(skills): enhance prompt architect with strategic snapshot); dai-vault main 1a79712 before -> docs commit (docs(prompting): add strategic snapshot and opportunity cost). Not pushed. Next: the proposed Directional-Contrast Multi-Slate Settlement + Capture Continuation v1 (06 Execution/prompting/next-slice.md), validated against live repo state by dai-slice-runner first.

---

status: Prompt Architect Skill Test v1 complete 2026-06-24 -- VALIDATION/REVIEW slice, no execution, skills + docs only. Ran through dai-slice-runner + dai-skill-router gate + dai-slice-prompt-architect (dry MODE 1/MODE 2 review) + dai-token-tight + superpowers:verification-before-completion. Tested dai-slice-prompt-architect against the live next-slice.md proposal WITHOUT executing it. Checklist result: verified-starting-state (stale, see finding), goal (pass), skills (pass), non-goals (pass), Strategic Snapshot (pass -- 6 compact substantive fields), Opportunity Cost (pass -- real alt Calibration Read v3, deciding factor stated), validation checklist (pass), authority boundary (pass -- proposal-only, runner-gated), handoff shape (pass). Risk scan: prompt bloat none; decorative rigor none; strategy laundering low (Revenue Path is a labeled prioritization input, not asserted strategy); accidental execution authority none; duplicated doctrine minor/acceptable (truth hierarchy intentionally in both skill + doctrine); overfitting sports lessons globally none (each sports lesson carries explicit when-not-to-reuse scoping). ONE REAL FINDING: stale repo-state assumption -- next-slice.md expected heads (1a7c84c/5c9ff79) were exact-match and had gone stale after two skills-only commits advanced dai/vault to ba0e611/498de2a, which would falsely flag divergence and trigger unnecessary re-derivation. SMALLEST FIX APPLIED: (1) next-slice.md now states expected heads as "at or after" + a MATERIAL PRECONDITION (cohort-v2 unsettled + corpus 47/47) and marks skills/docs-only drift immaterial; validation-checklist head item softened to materiality-based. (2) Skill MODE 5 generalized to require a material precondition and "at or after" head phrasing so future proposals are not invalidated by unrelated commits. VERDICT: PASS WITH MINOR EDITS. What did NOT change: no proposed slice executed; no runtime code, product prompts, confidence, source-depth, advertised-strength, buyer copy, reconciliation, Tool Gateway, billing, auth, tenant; no automation; next-slice remains proposal-only; settled corpus 47/47. Files inspected: dai-slice-prompt-architect/SKILL.md, dai-skill-router/SKILL.md, next-slice.md, slice-prompt-architecture-doctrine-v1.md, prompt-patterns-and-lessons-v1.md, current-slice.md, dai-skills-inventory-v1.md. Files changed: dai-slice-prompt-architect/SKILL.md (MODE 5 materiality clause); next-slice.md (expected-heads materiality); current-slice.md (this entry). Repo states: dai main ba0e611 before -> skill commit (docs(skills): materiality precondition for next-slice proposals); dai-vault main 498de2a before -> docs commit (docs(prompting): make next-slice precondition materiality-based). Not pushed. Next: unchanged -- Directional-Contrast Multi-Slate Settlement + Capture Continuation v1 (now with a materiality-based precondition).

---

status: Directional-Contrast Multi-Slate Settlement + Capture Continuation v1 -- WAIT-ONLY 2026-06-24, settlement gate NOT met; 0 reconciled, no DB writes, corpus unchanged 47/47. Ran through dai-slice-runner + dai-skill-router gate + dai-grill-with-vault + superpowers:verification-before-completion. GATE CHECK (StatsAPI, ~17:26Z): the 14 cohort-v2 gamePks are tonight's slate -- 10 Scheduled, 2 In Progress (823850 Rangers@Marlins 1-1; 823613 Cubs@Mets 0-0), 1 Pre-Game, 1 Delayed Start; none Final. The only outstanding 200x game (200018/beb5433e, gamePk 823613) is the same In-Progress game. Zero Final -> zero reconcilable; no outcome guessed or posted (hard rule). MATERIAL PRECONDITIONS: newly-settled-games FALSE; reconciliation-data FALSE; corpus-uncontaminated TRUE (47/47 by sqlcmd); no-blocking-defect TRUE; cohort+market-baseline-accessible TRUE (14 active runs; baseline in capture-v2 doc). 2 of 5 preconditions fail -> wait-only by design, not error. PERFORMANCE/SCARCE-SIGNAL: not producible (no settled outcomes); restated pre-settlement structure only -- DAI matched market favorite 12/13, lone disagreement Brewers@Reds, null abstention Braves@Padres (the scarce games to grade once Final). READ V3 READINESS: NOT READY (corpus unchanged at 47; Read v2's >=3-balanced-slate bar unmet). Recommend: rerun the settlement pass after ~2026-06-25T04:00Z+ (cohort-v2 14 + 200018 on authoritative finals; measure DAI vs market/home/chance; isolate disagreement+null games), then capture the 06-25 slate; Read v3 only after >=3 settled balanced slates; no tuning. dai-grill-with-vault: consistent with the prior wait-only directional settlement precedent and the no-tuning-before-measurement doctrine. What did NOT change: no app source, no model/prompt/FastAPI, no schema, no confidence/lean/posture/source-depth/advertised-strength/reconciliation logic, no buyer copy, no Tool Gateway/probe/freshness/billing/auth/tenant; no DB writes; corpus 47/47. Doc: 04 Products/sports-v1/calibration/directional-contrast-multislate-settlement-v1.md. Repo states: dai main 166cb85 (untouched, synced); dai-vault main 6519631 before -> docs commit (this entry + the wait-only doc). Not pushed. Next: rerun the settlement pass after settlement (~2026-06-25T04:00Z+).

---

status: Settlement Readiness Report + Reconciliation Boundary Hardening v1 complete 2026-06-24 -- SYSTEM-IMPROVEMENT LANE, docs-only, no reconciliation, no writes, no tuning. Ran through dai-slice-runner + dai-skill-router gate + dai-grill-with-vault + superpowers:verification-before-completion. Prepared (did NOT perform) tomorrow's reconciliation. Baseline: dai 166cb85 clean; dai-vault 9a44f49 ahead 4; corpus 47/47. DECISION: docs-only (lowest-risk that materially helps), backed by a live READ-ONLY classification of the 15 staged runs; a read-only script/endpoint considered and DEFERRED (stable fields already resolved; finality is a one-line StatsAPI recheck tomorrow; committed live-DB tool not yet clearly justified). Built a settlement readiness report doctrine: (1) hard lane boundaries (reconciliation writes rows / system-improvement makes writes safer / tuning changes decisions -- never blur); (2) status taxonomy (final_ready, pending_not_final, postponed_or_suspended, delayed, already_reconciled, excluded, invalid, duplicate_risk, identity_risk, missing_market_baseline, missing_artifact, unknown_requires_manual_review) + eligibility + DAI-market relationship rules; (3) a pre-resolved 15-run inventory (read-only DB + StatsAPI); (4) tomorrow's reconciliation checklist + gate. STAGED INVENTORY (read-only): 15 runs (14 cohort-v2 + 200018/beb5433e), ALL outcome=no/eval=no (zero reconciled -> corpus integrity confirmed), all identity-bearing (mlb_statsapi+gamePk), all pending_not_final today (games not Final at 17:26Z). FLAGS: duplicate_risk = beb5433e + d879433e share gamePk 823613 (same rescheduled Cubs@Mets game, OPPOSITE leans away/home, activeSameGame=2) -> tomorrow reconcile both per-run by run id (never by identity -> MultipleMatches), do not double-count as independent observations; missing_market_baseline = beb5433e (no baseline at its 06-22 capture); scarce-signal disagreement = fe79433e (Brewers@Reds, DAI home vs market away); null_abstention = 077a433e (Braves@Padres). 13 prior exclusions remain excluded (not in staged list). VERIFICATION: corpus unchanged 47/47 before and after (only SELECTs + StatsAPI GETs issued; zero POST/writes); dai tree CLEAN (no code/decision-engine/prompt/model/confidence/source/buyer change); no new slate captured; no winners inferred; no partial scores used as performance. Doc: 04 Products/sports-v1/calibration/settlement-readiness-report-and-reconciliation-boundary-v1.md. Repo states: dai main 166cb85 (untouched); dai-vault main 9a44f49 before -> docs commit (this entry + the readiness doc). Not pushed. Next: DIRECTIONAL-CONTRAST SETTLEMENT PASS V1 after 2026-06-25T12:00:00Z -- use the readiness report to flip pending->final_ready, reconcile only Finals per-run, then measure DAI vs actual/market/home/chance.

---

status: Pre-Settlement Idempotency + Duplicate-Risk Manifest v1 complete 2026-06-24 -- SYSTEM-IMPROVEMENT LANE, docs-only, no reconciliation, no writes, no tuning. Ran through dai-slice-runner + dai-skill-router gate + dai-grill-with-vault + superpowers:verification-before-completion. STATE NOTE: prompt expected dai-vault ahead 5 unpushed, but it was pushed last turn -- both repos synced with origin at slice start (dai 166cb85, dai-vault cdd4b1c), benign discrepancy. Baseline corpus 47/47; all 15 staged runs verified outcome=0/eval=0 (idempotency start state clean, 0 already-reconciled). DECISION: docs-only (no code; the write rules + frozen scope are doctrine, not a tool). Created pre-settlement-idempotency-duplicate-risk-manifest-v1.md with: (1) IDEMPOTENCY DOCTRINE -- reconciliation idempotent by agentRunId (never gamePk/team/date); pre-write check for existing outcome/eval; both-exist->skip already_reconciled (API also 409s); exactly-one-exists->partial_reconciliation_risk/unknown_requires_manual_review, do NOT blindly write; neither+eligible->write; use per-run POST /{id}/outcome, never the (provider,gamePk) matcher for the dup group. (2) DUPLICATE-RISK DOCTRINE -- duplicate-risk != invalid; game identity gives the SCORE, agentRunId gives the evaluable DECISION; reconcile per run id, never collapse by gamePk, never silently drop, metrics must state how same-game runs were handled, a read must not treat two reads of one game as independent direction observations. (3) FROZEN MANIFEST of exactly 15 runs (full agentRunIds) -- the ONLY runs tomorrow may reconcile; 13 prior exclusions NOT included; per-run fields incl DAI-market relationship. MARKINGS: fe79433e Brewers@Reds = DAI-market disagreement (high-info); 077a433e Braves@Padres = null abstention (high-info); G1 = d879433e + beb5433e share gamePk 823613, opposite leans, reconcile per agentRunId, do not collapse; beb5433e = market_missing -> out of DAI-vs-market denominator but eligible for actual/home/chance. SCOPE RULE: tomorrow may reconcile only manifest runs unless a later slice expands scope. VERIFICATION: corpus 47/47 before==after (only SELECTs, zero writes); no winners inferred; no partial scores as performance; dai tree CLEAN (no code/decision change); manifest = exactly 15, exclusions preserved, dup group + market-missing + high-info documented. GATE: 2026-06-25T12:00:00Z or later. Doc: 04 Products/sports-v1/calibration/pre-settlement-idempotency-duplicate-risk-manifest-v1.md. Repo states: dai main 166cb85 (untouched); dai-vault main cdd4b1c before -> docs commit (this entry + the manifest). Not pushed. Next: DIRECTIONAL-CONTRAST SETTLEMENT PASS V1 after the gate -- reconcile only frozen-manifest Finals per agentRunId (idempotent), keep beb5433e out of the market denominator, then measure DAI vs actual/market/home/chance.

---

status: Pre-Settlement Idempotency Guard Verification + Hardening v1 complete 2026-06-24 -- code + tests (reconciliation safety only), no reconciliation, no live writes, no tuning. Ran through dai-slice-runner + dai-skill-router gate + systematic-debugging + dai-test-discipline + dai-grill-with-vault + verification-before-completion. WRITE-PATH INSPECTION: per-run POST /api/agent-runs/{id}/outcome and POST /reconcile both write via the shared AddOutcomeAndEvaluation (AgentRunsController.cs) which stages AgentRunOutcome + AgentRunEvaluation in ONE SaveChangesAsync (atomic -> no partial state from the app); idempotency existence check was AgentRunOutcomes.AnyAsync(AgentRunKey==run.AgentRunKey) -> 409, keyed by AgentRunKey (1:1 with agentRunId), NEVER gamePk; the reconcile matcher returns MultipleMatches without writing (same-gamePk runs are not collapsed). CLASSIFICATION: C -- idempotent by agentRunId and correct for every app-reachable state, but the existence check inspected only outcomes, so an orphan-evaluation partial state (unreachable via the atomic path, possible via out-of-band/manual edit) would not block a duplicate write. HARDENING (smallest safe, deterministic): broadened the existence check in BOTH RecordOutcome and Reconcile to 409 if an outcome OR an evaluation already exists for the AgentRunKey (still keyed by run, never gamePk; no schema change; happy path unchanged). GUARD LOCATION: DevCore.Api/Controllers/AgentRunsController.cs RecordOutcome (~L505) + Reconcile (~L663). PARTIAL-STATE: orphan eval -> 409 (manual review), writes nothing; orphan outcome -> 409 (already covered); the atomic write means neither arises normally. DUPLICATE-RISK: confirmed not collapsed -- different agentRunIds sharing one gamePk each get their own outcome+eval, graded on each run's own lean. TESTS ADDED (DevCore.Api.Tests/Integration/AgentRunsControllerTests.cs, in-memory EF, no live DB): record_outcome_twice_does_not_create_duplicate_outcome_or_evaluation_rows (retry -> 409, counts stay 1/1); two_runs_sharing_a_gamepk_are_reconciled_per_run_and_not_collapsed (gamePk 823613 home+away seeds -> 2 outcomes + 2 evals, home eval correct / away eval incorrect); record_outcome_returns_409_when_only_an_evaluation_exists_for_the_run (orphan eval -> 409, 0 outcomes written). Existing 409/201/eval + matcher MultipleMatches tests retained. CHECKS: scoped 7/7 green; declared final verification full DevCore.Api.Tests 894/894 (was 891 +3). LIVE SAFETY: live devcore corpus 47/47 before==after (tests are in-memory; zero live POST); no staged run reconciled; no performance metric produced; no decision-engine/prompt/model/lean/confidence/source-threshold/advertised-strength/buyer-copy/market-weighting/artifact-semantics file changed (RunEvaluator untouched); G1 handled per agentRunId (proven by test); beb5433e remains market-denominator ineligible (doc-level, unchanged). Files changed: DevCore.Api/Controllers/AgentRunsController.cs (guard) + DevCore.Api.Tests/Integration/AgentRunsControllerTests.cs (3 tests). Repo states: dai main 166cb85 before -> code commit (fix(calibration): harden reconciliation idempotency by agent run id); dai-vault main 19eefd5 before -> docs commit (this entry). Not pushed. Next: DIRECTIONAL-CONTRAST SETTLEMENT PASS V1 after 2026-06-25T12:00:00Z -- reconcile only frozen-manifest Finals per agentRunId (the verified+hardened guard now protects against duplicate/partial writes), keep beb5433e out of the market denominator, measure DAI vs actual/market/home/chance.

---

status: Supported Sport Availability UX Investigation v1 complete 2026-06-24 -- INVESTIGATION ONLY, working as designed, NO code change. Ran through dai-slice-runner + dai-skill-router gate + systematic-debugging + product-ui-design-architect + verification-before-completion. ROOT CAUSE (verified in source): football is hidden INTENTIONALLY because NFL/NCAAF are IsBuyerReady=false / smoke_level_only in the backend catalog (CompetitionCatalog.cs, Buyer Packaging Source-of-Truth v1: "NFL/NCAAF/NCAAMB are routable but not buyer-ready (smoke-level only)"); the frontend filter buyer-ready-competitions.ts keeps only isBuyerReady===true and analyzer.component.ts:396 applies it, so the selector = buyer-ready sport families = Baseball(MLB)+Basketball(NBA). The selector is driven by BUYER-READINESS, not game availability (NBA shows even with no NBA games). Existing test buyer-ready-competitions.spec.ts already LOCKS this with comp('nfl', false, 'football') -> dropped. Empty-states for no-games already exist (analyzer.component.ts:222 "No upcoming matchups found"; :115 "No games on the books for this pairing"). DECISION: no change -- the slice's conditional ("football buyer-supported but hidden ONLY due to no games") is FALSE; football is not buyer-ready, so showing it would (a) require flipping backend IsBuyerReady (forbidden sport-support-semantics change, no defect), (b) violate "do not show internal/dev-only sports" and overclaim support (the slice's Primary Risk). UX CONTRACT (current, already correct): selector shows buyer-ready sports; game/date area shows available games with a graceful empty-state; internal/smoke-only sports hidden; no live-now overclaim. SUCCESS CRITERION re-read: the product already distinguishes buyer-supported sports from available games; football correctly does NOT appear because it is not buyer-supported (smoke-only) -- the football-should-appear premise was a misdiagnosis. RECOMMENDED FOLLOW-UPS (deferred): (1) Football Buyer-Readiness Validation -- a product+platform decision to validate football and set NFL IsBuyerReady=true (then it appears automatically, no frontend change); (2) optional no-games empty-state polish into a card (decoupled from football; applies to NBA offseason etc.). VERIFICATION: dai tree CLEAN (no code change); behavior proven by source trace + existing passing spec; live UI run unnecessary (filtering path is deterministic in source -- the authority). No decision-engine/calibration/buyer-copy files changed; no buyer-copy overclaim introduced. Doc: 04 Products/sports-v1/supported-sport-availability-ux-investigation-v1.md. Repo states: dai main 166bc19 (untouched; 1 prior unpushed commit preserved); dai-vault main 417cf6d before -> docs commit (this entry + the investigation note). Not pushed. Next: only if product chooses to offer football -- Football Buyer-Readiness Validation (set NFL IsBuyerReady=true after real validation); otherwise none (no UX defect).

---

status: Cognitive Factory Runtime Activation Readiness v1 complete 2026-06-25 -- READ-ONLY architecture assessment; NO runtime/flag/wiring/behavior change. Verified all claims against runtime source + DI + config (file:line), not docs alone. Key finding: TWO bodies of runtime -- (1) the Tool Gateway is LIVE on every sports run (AddDaiToolGateway Program.cs:164; SportsRetriever/SportsAnalyzer/SportsReferenceController callers; AgentRunService:43/52), enforcing fail-closed authorization (ToolGateway.cs:59-72) + per-call audit logging, but only 2 of 5 cloud-runtime guarantees (rate-limit/timeout/idempotency/tenant-tier absent) -> PARTIALLY READY; (2) the Protocol stations + 17-seam Probe Refresh chain are DORMANT -- DI-wired but no live caller, every master switch default-OFF (ChainAssembly.Enabled/AllowGatewayExecution/PersistAuditRecord false ProbeRefreshChainAssembly.cs:76-78; ProbeRefreshExecutor registered Disabled Extensions:61-63), NONE bound to IConfiguration (so activation is a source edit, not a config flip), no protocol/probe HTTP endpoint, artifact mutation structurally impossible (no writer), DiscernStationRunner not even registered. Readiness answer: NO for any mutating/decision activation; the single blocking capability is a config-bound reversible flag + a runtime observability surface. Recommended smallest-first activation order: Layer 0 read-only diagnostics endpoint (executes nothing) -> Layer 1 config-bound default-off flags -> Layer 2 dev-only audit-only non-mutating probe run; STOP before Layer 3 (merge writer), which is gated by Evidence Readiness Gate 4/5 (not met). Deliverable: 02 Platform/architecture/cognitive-factory/cognitive-factory-runtime-activation-readiness-v1.md (+ related-docs link from evidence-readiness-gates-v1). Next recommended slice: Cognitive Factory Observability Surface v1 (implement the Layer-0 read-only endpoint). No tuning; no code activated.

---

status: Cognitive Factory Observability Surface v1 complete 2026-06-25 -- IMPLEMENTED (code-changing slice; TDD; dai-code-reviewer; full suite 911/0). Built the Layer-0 / Stage-0 activation-ladder surface: dev-only read-only GET /api/dev/cognitive-factory/diagnostics (404 in Production/Staging; anonymous-in-dev by deliberate decision -- non-sensitive topology, executes nothing). New DevCore.Api/Diagnostics (CognitiveFactoryDiagnostics DTO + CognitiveFactoryReadiness enum + CognitiveFactoryDiagnosticsProvider) + Controllers/Dev/CognitiveFactoryDiagnosticsController + AddSingleton in Program.cs. Provider is a pure projection: di-registration probe via IServiceProviderIsService.IsService (NO construction), enabled flags grounded in compiled ProbeRefresh*Options.Disabled defaults, plus a static inventory verified against cognitive-factory-runtime-activation-readiness-v1. Reports 13 components with implemented/registered/enabled/executable/activationLayer/readiness(Ready|PartiallyReady|NotReady|LivePartial)/status/blockingRequirements/notes. Tests (8): dev 200 + no db write (incl ProbeRefreshMergeAudits), Production + Staging 404, ToolGateway LivePartial, executor disabled, discern unregistered, flags not config-bound, and no-execution (throwing node-runner + throwing gateway-capable executor factory -> still 200). Executed NO cognitive runtime; touched no tool/probe/model/artifact/outcome/calibration row; enabled no flag; changed no settlement/reconciliation/buyer/frontend behavior. Docs: cognitive-factory-observability-surface-v1.md (+ reconciled readiness-v1 Originally/Today). Skill: dai-slice-prompt-architect sec 18 (activation ladder) + inventory note. Commits: dai 737abfd (code) + d1bcd32 (skill); vault ebeab22 (docs). Next slice: Cognitive Factory Configuration-Bound Control v1 (Stage 1 -- bind options to IConfiguration, default off; surface bound values; no execution).

---

status: Cognitive Factory Configuration-Bound Control v1 complete 2026-06-25 -- IMPLEMENTED (Stage 1 of activation ladder; code-changing; TDD; dai-code-reviewer; full suite 915/0). Bound a default-off activation posture from IConfiguration section "CognitiveFactory" (new CognitiveFactoryActivationOptions: Enabled/AllowExecution/AllowToolExecution/AllowArtifactMutation/AuditOnly/ActivationStage/EnvironmentScope; all safe default-off, AuditOnly default on) via Program.cs Configure<>. NO runtime consumer added -- config is observable (diagnostics ConfiguredActivation block) and reversible (config), not executable: ExecutionBlocked/ToolExecutionBlocked/ArtifactMutationBlocked hardcoded true in Stage 1. Endpoint stays dev-only/read-only; activationStage now 1; cognitiveRuntimeActivated false. Tests (12; +4): missing->off, false->off, true->observable-but-blocked + executor still non-executable + runtime not activated, config-bound status reported, posture not observable outside dev even when true. Executed NO cognitive runtime; no probe refresh; no tool/model/db/artifact/outcome/calibration touched; no flag enabled by default; no settlement/reconciliation/buyer/frontend change. Docs: cognitive-factory-configuration-bound-control-v1.md (+ reconciled observability-surface-v1 "what remains before Stage 1" = done). Skill: dai-slice-prompt-architect sec 18 Stage-1 checklist + inventory note. Commits: dai 6ef7ae5 (code) + skill commit; vault docs commit. Next slice: Cognitive Factory Read-Only Execution v1 (Stage 2 -- first dev-only consumer reads AllowExecution+AuditOnly to run one audit-only non-mutating probe; reversible by config; no mutation/Gate-4-5).

---

status: Cognitive Factory Read-Only Execution v1 complete 2026-06-25 -- IMPLEMENTED (Stage 2 of activation ladder; first controlled runtime execution; code-changing; TDD; dai-code-reviewer CLEAN; full suite 928/0). New dev-only POST /api/dev/cognitive-factory/probe/audit-only: executes ONLY when env.IsDevelopment() AND config gates pass (Enabled && AllowExecution && AuditOnly && !AllowArtifactMutation, pure CognitiveFactoryExecutionGate), then runs exactly one deterministic ProtocolNodeRunner.ExecuteAsync(interrogate.probe) -- no model/tool/gateway/external call, persists nothing, mutates no artifact/decision. Refusal -> 200 structured result naming the blocked gate. Per Part G, NO db write: existing ProbeRefreshMergeAudits schema is merge-specific, so result is in-memory only (AuditPersisted=false, DbWritten=false; durable audit deferred). Diagnostics gains ReadOnlyExecution block (endpointExists, auditOnlyExecutionPermittedByConfig from pure gate, mutation/tool/external/model all blocked, dbWritesByExecution false) and reports activationStage 2; computing it executes nothing. New files: Diagnostics/CognitiveFactoryExecutionGate.cs (gate + result DTO), Controllers/Dev/CognitiveFactoryExecutionController.cs; +ReadOnlyExecution to diagnostics DTO/provider; tests CognitiveFactoryExecutionGateTests + endpoint tests in CognitiveFactoryDiagnosticsTests. Tests (13): gate refusal/permit; prod 404 even with valid config; refused default-off / when AllowArtifactMutation true; permitted audit-only writes nothing (outcomes/evals/merge-audits 0); throwing IToolGateway still 200 (no tool/external); diagnostics stage-2 capability + permitted-when-safe. NO artifact/decision mutation; no model/external/tool call; no settlement/reconciliation/buyer/frontend change; no flag enabled by default; no tuning. Docs: cognitive-factory-read-only-execution-v1.md (+ reconciled config-bound-control-v1 "what remains before Stage 2" = done). Skill: dai-slice-prompt-architect sec 18 Stage-2 checklist + inventory. Commits: dai a6c1bc5 (code) + skill commit; vault docs commit. Next slice: NOT Stage 3 (locked by Gate 4/5) -- either Cognitive Factory Audit Ledger v1 (durable non-decision audit for the audit-only path) or resume evidence track (Cohort Capture v3 + Market Baseline v2) toward Gate 4.

---

status: Prompt Ledger Hook v1 complete 2026-06-25 -- PROCESS + SKILL slice; docs + skill guidance only; NO runtime/app/Cognitive-Factory/Stage-2 change. Stage 2 confirmed already committed+pushed (dai a6c1bc5/9de1cd3, dai-vault f83e0d2) and left untouched. Created cross-cutting Prompt Ledger Hook: capture FINALIZED reusable slice prompts as compact Obsidian-friendly records, external-first at <OBSIDIAN_PROMPT_LEDGER_ROOT>/<PROJECT_KEY>/prompts/YYYY/MM/YYYY-MM-DD-slice.md (never forced into dai/dai-vault; never commit external Obsidian files unless the agreed folder exists; workspace has only dai/+dai-vault so the record is embedded as an example in the doc). Capture criteria: slice-initiating / architecture / activation-posture / reusable-pattern / skill / governance prompts; NOT drafts/dupes/minor-wording/casual/secrets; one record per slice. Timing: pre-execution (primary, before send), post-execution (link outcome), retroactive (backfill). Record format: frontmatter (date/project/slice/lane/activation_stage/prompt_type/agent_target/capture_mode/status/source/repo_target/related_docs/related_commits) + sections Prompt Summary/Guardrails/Final Prompt/Outcome (+optional Reusable Pattern/Skill Updates/Follow-Up). Doc: 06 Execution/prompt-ledger-hook-v1.md (incl. retroactive Stage-2 example, capture_mode: retroactive). Skills (composition, not inheritance): dai-slice-prompt-architect sec 19, dai-docs-architect pair-with, dai-agent-handoff pair-with; inventory updated. No external Obsidian files written. No secrets/draft capture. Commits: dai skills commit; vault docs commit. Next: adopt the hook -- the next finalized canonical prompt should produce a pre-execution record once the human confirms the ledger root location.

---

status: Directional-Contrast Cohort Capture v3 + Market Baseline v2 complete 2026-06-26 -- LANE A EVIDENCE ACQUISITION; generation + market-baseline + artifact-trace only; NO tuning, NO settlement, NO Cognitive Factory activation, NO code change. Ran through dai-slice-runner + dai-skill-router gate + verification-before-completion (dai-skill-router/dai-slice-runner are dai/.claude/skills scoped to dai/, loaded directly). PREFLIGHT: dai e629bac + dai-vault ed3ec7d both clean/synced (ff-only: already up to date); DevCore.Api COLD-BUILT+run from HEAD e629bac (not a stale binary); Docker 29.1.3 + devcore-sql up (db devcore present); FastAPI :8000 /api/ping ok; DevCore.Api :5007 /health ok; StatsAPI 15 games; Odds API 15 events (377 reqs left). Cognitive Factory INERT (no CognitiveFactory config section -> code defaults off; no diagnostics/execution endpoint called; Stage 2 NOT invoked; normal sports path is separate). SLATE: capture 12:57Z (08:57 ET), all 15 games Preview/not-started (earliest first pitch 22:40Z), 15 included / 0 excluded, all identity-traced (StatsAPI gamePk + odds event id). COHORT: 15 runs POSTed to /api/agent-runs (sports.matchup.analysis), 15/15 completed, all artifact v3, all mlb_statsapi, all ExclusionReason NULL, tenantKey 1, posture monitor. Lean home 10 / away 4 / null 1. DIRECTIONAL CONTRAST present (v3 goal v2 lacked): 12 enriched (richness 2 / advertised High / starter+market enriched) vs 3 thin (richness 1 / advertised Medium / starting_pitching none-missing: Phillies@Mets null, Royals@WhiteSox home, Braves@Giants away). 4 source envelopes/run (starting_pitching, market, availability, bullpen_fatigue) -- richer than v2's 2. MARKET BASELINE v2: the-odds-api v4 baseball_mlb h2h us, captured 12:57:07Z, per-book de-vig consensus; all 15 market-backed / 0 missing; market favorite home 9 / away 6 (home-heavy slate). DAI tracks market favorite 13/14 decided (93%); lone disagreement Yankees@Red Sox (DAI home/Red Sox vs market away/Yankees devig 0.514); null abstention Phillies@Mets (where a starter was missing). INTEGRITY SCAN: CLEAN, zero PotentialMismatch -- persisted LeanSide == artifact leanSide 15/15; artifactDirectionConsistency 14 Consistent + 1 NotEvaluable (null lean, correct); identity/version/envelopes present 15/15; no duplicate gamePks; market linkage valid 15/15. PROMPT LEDGER: pre-execution capture-worthy; OBSIDIAN_PROMPT_LEDGER_ROOT unset + no agreed in-vault ledger folder -> NO external prompt-ledger file written (per hook fallback, embedded compact record in the cohort doc sec 9). VERIFICATION: corpus outcomes/evaluations UNCHANGED (no /outcome POST; SELECTs + StatsAPI/Odds GETs + agent-run POSTs only); dai tree CLEAN (no code changed; git diff --check clean); settlement-ready but UNSETTLED (settle ~2026-06-27T04:00Z+). Docs: 04 Products/sports-v1/calibration/directional-contrast-cohort-v3-market-baseline-v2.md + machine manifest artifacts/20260626-directional-contrast-cohort-v3-manifest.json + this entry. Repo states: dai main e629bac (untouched, synced); dai-vault main ed3ec7d before -> docs-only changes (this entry + cohort doc + manifest). Not committed/pushed unless instructed. Next: SETTLEMENT PASS after 2026-06-27T04:00Z+ -- reconcile the 15 runs per agentRunId on StatsAPI Finals (graded on persisted LeanSide), measure DAI vs market/home/chance with the Yankees@Red Sox disagreement + Phillies@Mets null isolated; pool >=3 settled directional-contrast slates before Calibration Assessment v4; no tuning before a validated discrimination target.

---

status: Prompt Ledger Hook State Audit + Mind Map v1 complete 2026-06-26 -- PROCESS/SKILL ARCHITECTURE AUDIT; read-only over skills+vault+runtime; NO runtime/app/Cognitive-Factory change, NO new behavior, NO prompt-capture expansion, NO skill edits (no gap found). Ran through dai-slice-runner + dai-skill-router gate + dai-grill-with-vault + dai-docs-architect + dai-slice-prompt-architect + verification-before-completion + dai-agent-handoff. FINDINGS: Prompt Ledger Hook v1 is COMPLETE and process-layer-only. Canonical doctrine 06 Execution/prompt-ledger-hook-v1.md present with all 8 elements (purpose/storage/criteria/timing/format/privacy/retroactive/bloat). Invoked by composition from 3 skills: dai-slice-prompt-architect sec 19 (capture decision + capture_mode pre/post/retroactive + ledger-ready summary; no drafts/secrets; external-first), dai-docs-architect Pair-with (capture when a docs/milestone prompt materially changes architecture), dai-agent-handoff Pair-with (link outcome/commits at slice close). Skills inventory notes it (dai-skills-inventory-v1.md:186-188). RUNTIME ISOLATION VERIFIED: grep prompt.?ledger|OBSIDIAN_PROMPT_LEDGER|ledger.?root across dai/platform+apps+services (cs/py/ts/json/csproj) = 0 matches; keyword lives only in the 3 skill SKILL.md files + vault docs -> no endpoint, no DB table, no Obsidian writer, no runtime hook. CLASSIFICATION: COMPLETE (not PARTIAL, not RUNTIME-COUPLED RISK, not minor-doc-gaps). GAPS: none correctable; 2 non-blocking deferred observations -- (1) OBSIDIAN_PROMPT_LEDGER_ROOT unset (operational, by design; doctrine handles via embed fallback; awaits human ledger-root config), (2) embed-location nuance (hook doc vs slice deliverable doc) -- optional one-line clarification, NOT fixed to avoid scope creep. Deliverable: 06 Execution/prompt-ledger-hook-state-audit-v1.md (objective, files inspected, verified hook points table, lifecycle timing map, mermaid mind-map + plain outline, isolation result, storage model, capture modes, classification, gaps, recommendations, verdict). Did NOT duplicate the canonical doc -- referenced it. VERIFICATION: dai tree CLEAN (no code touched); dai-vault changes docs-only (this entry + audit doc); git diff --check clean. Repo states: dai main e629bac (untouched, synced); dai-vault main 86ddf32 before -> docs-only commit (this entry + audit doc). Not pushed unless instructed. Next: optionally configure OBSIDIAN_PROMPT_LEDGER_ROOT so the next finalized canonical prompt produces the first real external pre-execution record (no skill change needed -- wiring exists).

---

status: Prompt Ledger Root Configuration + Placeholder Hardening v1 complete 2026-06-26 -- CONFIG + DOCS slice; NO runtime/app/Cognitive-Factory change, NO skill-behavior change, NO code change. Ran through dai-slice-runner + dai-skill-router gate + dai-grill-with-vault + dai-docs-architect + verification-before-completion + dai-agent-handoff. GOAL: set OBSIDIAN_PROMPT_LEDGER_ROOT, hidden behind a placeholder like other paths. DECISION (user-confirmed via question): ledger root = a `prompt-ledger` folder SIBLING to dai/ and dai-vault/ under the workspace root (outside both git repos) -- chosen over the literal workspace root because the convention <ROOT>/<PROJECT_KEY>/prompts would otherwise route records into dai/ (doctrine: do not force the ledger into dai/dai-vault). MECHANISM (matches dai/docs/examples/agent-paths.example.md): real literal paths live ONLY in the gitignored, uncommitted <DAI_WORKSPACE_ROOT>/.local/agent-paths.md; committed docs use the <OBSIDIAN_PROMPT_LEDGER_ROOT> placeholder. CREATED (uncommitted, outside both repos): .local/agent-paths.md (real path map incl ledger root); prompt-ledger/dai/prompts/2026/06/ + prompt-ledger/README.md. COMMITTED CHANGES: dai/docs/examples/agent-paths.example.md (added <OBSIDIAN_PROMPT_LEDGER_ROOT> placeholder line + sibling-folder guidance, example path C:\Path\To only); dai-vault prompt-ledger-hook-v1.md (section A "Configured (2026-06-26)" note + recommended-next-slice updated: root now confirmed, wiring already exists, no skill change); dai-vault prompt-ledger-hook-state-audit-v1.md reconciled Originally-unset/Today-configured across storage note + gap#1 + recommendations + verdict. VERIFICATION: literal path C:\Users\trolo\source\repos\dai-workspace = 0 matches in dai tracked files and 0 in my new/edited vault files; example uses only C:\Path\To; .local/ + prompt-ledger/ confirmed outside both git trees. FINDING (pre-existing, NOT introduced here): the literal workspace path is already hardcoded in 2 committed dai-vault files -- README.md (lines 133/136/137) and 04 Products/sports-v1/stage-0-true-calibration-candidate-capture-v1.md (lines 9/221). Recommend a small follow-up placeholder-hardening pass to swap those to placeholders if full hiding is desired; left untouched here to avoid scope creep + preserve historical-doc fidelity. Repo states: dai main e629bac before -> docs commit (example file); dai-vault main 575baa1 before -> docs commit (hook doc + audit doc + this entry). Not pushed unless instructed. Next: optionally (a) harden the 2 pre-existing literal-path leaks, and/or (b) write the first real pre-execution ledger record for the next finalized canonical prompt at <OBSIDIAN_PROMPT_LEDGER_ROOT>/dai/prompts/2026/06/.

---

status: System State Synopsis v1 complete 2026-06-26 -- DOCS-ONLY orientation slice; NO runtime/app/Cognitive-Factory change, NO code change, NO tuning, NO settlement, NO outcomes/evaluations written, NO Cognitive Factory activation changed, NO external prompt-ledger record written. Audit-before-acting discipline (read handoff + vault + verified source anchors before asserting). NOTE: the dai-specific skills named in the slice (dai-slice-runner, dai-skill-router, dai-grill-with-vault, dai-docs-architect, dai-slice-prompt-architect, dai-agent-handoff) are NOT installed in this environment -- ran directly under their discipline; verification-before-completion applied at close. PREFLIGHT: dai main fee495d clean/synced (0/0); dai-vault main 6218aab clean/synced (0/0); current-slice.md fully reconciled through Root Configuration v1 (no stale tail). FINDINGS: produced a single current-state map across 3 lanes. Lane A (Evidence) -- corpus 59 valid (settled AND ExclusionReason IS NULL); Calibration Assessment v3 is the frozen baseline (DAI 57.7% decided, inside noise vs 51.9% home base; confidence non-predictive; readiness-to-tune NO); Cohort v3 (15 MLB, 2026-06-26) captured/integrity-clean/market-backed, 0 outcomes written, SETTLEMENT-READY but UNSETTLED (settle ~2026-06-27T04:00Z+); Calibration v4 deferred until >=1 (pref >=3) settled slates. Lane B (Cognitive Factory) -- Stage 0 observability + Stage 1 config-bound (default-off, non-executable) + Stage 2 dev-only audit-only execution all COMPLETE; activationStage 2, cognitiveRuntimeActivated false; Tool Gateway live-partial (2/5 cloud guarantees); protocol/probe chain dormant; Stage 3 mutation LOCKED behind Evidence Readiness Gate 4/5. Lane C (Process) -- Prompt Ledger Hook canonical + development-time only + runtime grep 0 matches; ledger root CONFIGURED (prompt-ledger sibling folder outside both repos; literal path only in gitignored .local/agent-paths.md); audit + mind-map COMPLETE; 2 pre-existing literal-path leaks noted (README.md + stage-0-true-calibration-candidate-capture-v1.md), left untouched. Gates: 1-3 cleared, 4 NOT achieved, 5 locked. VERIFICATION: git diff --check clean; git status clean except the new synopsis doc + this entry (both docs-only); no app code changed; no runtime behavior changed; no Cognitive Factory behavior changed; no settlement/reconciliation/calibration/buyer/frontend change. Deliverable: 06 Execution/system-state-synopsis-v1.md (objective, repo state, lane summaries, active/dormant/pending table, risks, ranked next actions, mermaid mind map + outline, final verdict, related-doc anchors -- prior docs referenced, not duplicated). Repo states: dai main fee495d (untouched, synced); dai-vault main 6218aab before -> docs-only changes (synopsis doc + this entry). Not committed/pushed unless instructed. Next recommended: SETTLE Cohort v3 after games final, then settlement comparison (not Calibration Assessment v4 yet); continue market-backed captures; optionally Interrogate Statistical Inference Readiness v1 (read-only); defer all tuning until Gate 4.

---

## Settlement Reconciliation + Cohort Capture v4 + FIFA Feasibility v1 -- complete 2026-06-28

**Lane A -- Cohort v3 settled.** All 15 games (2026-06-26 slate) Final; reconciled per agentRunId via
`POST /api/agent-runs/{id}/outcome` + `GET .../evaluation` (server-graded on persisted LeanSide; StatsAPI finals,
never guessed). Result: 15/15 settled, **10 correct / 4 incorrect / 1 inconclusive** (DAI 10/14 decided = 71.4%; the
null Phillies@Mets graded inconclusive). The lone DAI-vs-market disagreement (Yankees@Red Sox) resolved in DAI's
favor (n=1). Idempotency verified (re-run = 15x 409). DB cross-check: 15 outcomes + 15 evals (1 each), 0 dup, 0
excluded. **Valid denominator 59 -> 74** (settled AND ExclusionReason IS NULL). Doc:
`04 Products/sports-v1/calibration/directional-contrast-cohort-v3-settlement-v1.md`.

**Lane B -- Cohort v4 captured.** 15 pregame MLB runs for 2026-06-28 via the normal pipeline (no Cognitive Factory),
all richness 2, all market-backed (de-vig consensus, 9 books each), 15/15 stable identity, **0 outcomes written**.
Lean 12 home / 3 away / 0 null. **Integrity blocker (documented, not mutated):** 1 PotentialMismatch run --
4f37433e Phillies@Mets (structured lean home, prose leans Phillies/away). Left as-is + flagged per the 722D
precedent (mismatch-remediation-v1 sec 6); the live 422 settlement guard blocks it from grading. DAI-vs-market:
12 agree / 3 disagree (one disagreement is the mismatch-confounded run; clean disagreements = 2). Docs:
`.../calibration/directional-contrast-cohort-v4-market-baseline-v3.md` +
`.../calibration/artifacts/20260628-directional-contrast-cohort-v4-manifest.json`.

**Lane C -- FIFA World Cup feasibility (read-only).** Conclusion: CONDITIONAL-GO for an internal sample spike behind
a new competition config; NO-GO for any buyer-facing support. Market odds available (the-odds-api
`soccer_fifa_world_cup` active); identity model fits `(SourceProvider,ExternalGameId)`; draw outcome already modeled
(-> inconclusive). Hard gaps: two-way de-vig + LeanSide (no draw side); no soccer analyzer/prompt/context; no soccer
results source; knockout regulation-vs-advancement semantics not modeled. No samples captured (would 400). Doc +
next-slice prompt: `03 Niches/sports-analytics/fifa-world-cup-feasibility-v1.md`.

**Discipline.** No tuning; no prompt/confidence/model/posture/buyer/Cognitive-Factory change. No `dai` code change
(existing endpoints + ephemeral scratchpad tooling only). Attribution guard: no co-author/Claude/AI trailers (global
