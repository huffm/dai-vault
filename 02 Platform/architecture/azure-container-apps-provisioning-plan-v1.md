...
- **The customer identity provider is Microsoft Entra External ID (CIAM), confirmed.** Full plan in `customer-auth-readiness-v1.md`; this section is the provisioning-side summary. External ID is a single JWT issuer that federates Google, Apple, Microsoft, and email; no per-method code branching.
- `dai-api` today wires JWT bearer auth in `Program.cs:42-53` with authority `https://login.microsoftonline.com/{AzureAd:TenantId}/v2.0` and audience `AzureAd:Audience`. That is the workforce host and is the wrong host for customers. The auth slice (not this slice) makes the authority configurable by adding `AzureAd__Instance` (e.g. `https://{tenant-subdomain}.ciamlogin.com/`) and building the authority as `{Instance}{TenantId}/v2.0`, pointing it at the External ID tenant. Set `AzureAd__Instance`, `AzureAd__TenantId`, and `AzureAd__Audience` (the `api://{api-app-id}` from the API app registration) on `dai-api`; all three are plain config, not Key Vault secrets. External ID public-client SPA sign-in introduces no client secret.
...
- **Gating rule:** the public `dai-api` ingress must not be exposed with bypass on or with no working customer identity. The External ID tenant, user flow (Google/Apple/Microsoft/email), and both app registrations (API + SPA), plus the auth slice (configurable authority, `idp` normalization, JIT provisioning, `[Authorize]` on `AgentRunsController`, MSAL in the sports-app), must be in place before public exposure. Full gate list in `customer-auth-readiness-v1.md` section 10. This is a launch blocker, not a provisioning detail.
...
- **Customer auth (gating for public exposure).** The provider is decided: Microsoft Entra External ID (`customer-auth-readiness-v1.md`). Stand up the External ID tenant, the Google/Apple/Microsoft/email user flow, and both app registrations (API + SPA), then run the auth implementation slice (configurable authority, `idp` normalization, JIT provisioning, `[Authorize]` on `AgentRunsController`, MSAL in the sports-app). Public `dai-api` ingress must not go live before this is working with `Dev__EnableBypassAuth=false`. Full gate list: `customer-auth-readiness-v1.md` section 10.
