...
- **Current choice:** contract only. `PerceiveSignalIntake`, `PerceiveSignalObservation`, `PerceiveSignalContext`, `PerceiveSignalSource`, `PerceiveSignalOrigin`, and `PerceiveSignalIntakeResult` exist in DevCore.Api.Protocols. A one-way projection from `PerceiveRefreshView` / `ProbeRefreshPerceiveIntakeResult` exists for compatibility proof. Analyzer seed parsing, `CognitiveProtocol`, `PerceiveProtocol`, `ProbeRefreshPerceiveIntake`, and the probe-refresh chain are not rewired.
...
- **Revisit trigger:** a second source needs to feed Perceive through the same normalized shape, or a future analyzer-seed adoption slice needs run/tenant/correlation/source metadata in a generic contract.
- **Proposed future slice:** Perceive Signal Intake Adoption v1 -- wire read-only projections from analyzer seed and probe-refresh into the generic contract without model calls, Tool Gateway calls, artifact writes, confidence/posture/lean mutation, endpoints, schema changes, FastAPI prompt changes, or Angular changes.
...
- **Status:** Deferred -- contract/projection shipped, dormant. Created by Perceive Signal Intake Standardization v1 (2026-06-06). Do not mark direct Interrogate -> Perceive refresh, activation, merge writer, artifact mutation, confidence/posture/lean mutation, memory/pgvector, Kubernetes/AKS, tenant/Stripe, or calibration threshold changes resolved from this entry.
