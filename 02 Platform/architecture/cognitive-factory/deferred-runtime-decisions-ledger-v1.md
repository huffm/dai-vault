...
- **Current choice:** projection support only. `PerceiveSignalIntake`, `PerceiveSignalObservation`, `PerceiveSignalContext`, `PerceiveSignalSource`, `PerceiveSignalOrigin`, and `PerceiveSignalIntakeResult` exist in DevCore.Api.Protocols. One-way projections now exist for both analyzer seed/model-emitted Perceive fields (`AnalyzerSeedPerceiveSignalProjection`) and probe-refresh/platform-refreshed Perceive views (`ProbeRefreshPerceiveSignalProjection`). Analyzer seed parsing, `CognitiveProtocol`, `PerceiveProtocol`, `ProbeRefreshPerceiveIntake`, and the probe-refresh chain are not rewired.
