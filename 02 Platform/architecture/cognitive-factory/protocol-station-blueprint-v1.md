...
There are 15 stations, one per node: Perceive {Detect, Frame, Aim}, Interrogate {Question, Probe, Verify}, Discern {Weigh, Contrast, Stress}, Decide {Resolve, Position, Justify}, Synthesize {Integrate, Compose, Deliver}. Twelve are cognitive (model-emitted today, inside one analyze call); three are deterministic platform stations (Probe, plus the Synthesize trio) with no model call. Retrieve is **not** a station; it is platform plumbing that runs before Perceive and stamps grounded evidence onto the artifact.
