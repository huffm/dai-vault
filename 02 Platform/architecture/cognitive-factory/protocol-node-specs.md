...
It is the contract a future code slice implements against. It does not describe 15 separate model calls. Today the 12 cognitive micro-actions (every node except Probe and the Synthesize trio) are produced by a **single analyze model call** in FastAPI; Probe and Synthesize are deterministic .NET. The node specs describe each node's *responsibility and boundary* regardless of how many calls implement them.
